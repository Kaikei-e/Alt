use rask_log_forwarder::reliability::{HealthMonitor, HealthConfig, HealthStatus, ComponentHealth};
...
async fn test_health_status_aggregation() {
    ...
    monitor.update_component_health("collector", ComponentHealth::Healthy).await;
    monitor.update_component_health("parser", ComponentHealth::Healthy).await;
    monitor.update_component_health("sender", ComponentHealth::Healthy).await;
...
async fn test_degraded_health_status() {
    ...
    monitor.update_component_health("collector", ComponentHealth::Healthy).await;
    monitor.update_component_health("parser", ComponentHealth::Degraded("High memory usage".to_string())).await;
    monitor.update_component_health("sender", ComponentHealth::Healthy).await;
...
async fn test_unhealthy_status_with_failures() {
    ...
    monitor.update_component_health("collector", ComponentHealth::Unhealthy("Cannot read logs".to_string())).await;
    monitor.update_component_health("parser", ComponentHealth::Healthy).await;
    monitor.update_component_health("sender", ComponentHealth::Healthy).await;
...
async fn test_get_all_component_status() {
    ...
    monitor.update_component_health("api", ComponentHealth::Healthy).await;
    monitor.update_component_health("database", ComponentHealth::Degraded("Slow queries".to_string())).await;
    monitor.update_component_health("cache", ComponentHealth::Unhealthy("Connection timeout".to_string())).await;
    ...
    assert!(matches!(all_status.get("database"), Some(ComponentHealth::Degraded(_))));
    assert!(matches!(all_status.get("cache"), Some(ComponentHealth::Unhealthy(_))));
}
...
async fn test_component_cleanup() {
    ...
    // Add some components
    monitor.update_component_health("temp_component", ComponentHealth::Healthy).await;
    monitor.update_component_health("permanent_component", ComponentHealth::Healthy).await;
    
    // Wait to simulate aging
    tokio::time::sleep(Duration::from_millis(10)).await;
    
    // Cleanup old components (very short max_age for testing)
    monitor.cleanup_stale_components(Duration::from_millis(5)).await;
