...
fn test_otlp_serialization_with_trace_context() {
    ...
    assert_eq!(
        log_record.trace_id, expected_trace_id,
...
fn test_otlp_serialization_resource_attributes() {
    ...
    if let Some(any_value::Value::StringValue(value)) = &service_name_attr
        .value
        .as_ref()
    ...
    if let Some(any_value::Value::StringValue(value)) = &container_id_attr
        .value
        .as_ref()
        .and_then(|v| v.value.as_ref())
    {
...
fn test_otlp_serialization_log_attributes() {
    ...
    if let Some(any_value::Value::StringValue(value)) =
        &custom_attr.value.as_ref().and_then(|v| v.value.as_ref())
    {
    ...
    if let Some(any_value::Value::StringValue(value)) =
        &method_attr.value.as_ref().and_then(|v| v.value.as_ref())
    {
        assert_eq!(value, "GET");
    } else {
...
fn test_otlp_serialization_timestamp() {
    ...
    assert!(
        log_record.time_unix_nano > 0,
        "time_unix_nano should be set"
    );
    assert!(
        log_record.observed_time_unix_nano > 0,
        "observed_time_unix_nano should be set"
    );
}
...
fn test_otlp_serialization_grouping_by_service() {
    ...
    assert_eq!(
        ...
        2,
        "Should have 2 ResourceLogs for 2 services"
    );
...
fn test_otlp_protobuf_smaller_than_json() {
    ...
    let protobuf_bytes = otlp_serializer
        .serialize_batch(&batch)
        .expect("OTLP serialization should succeed");
    let ndjson = json_serializer
        .serialize_ndjson(&batch)
