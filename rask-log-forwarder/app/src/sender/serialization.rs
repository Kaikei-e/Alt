...
impl BatchSerializer {
    ...
    pub fn serialize_batch_with_metadata(&self, batch: &Batch) -> Result<String, SerializationError> {
        if batch.is_empty() {
            return Err(SerializationError::EmptyBatch);
        }
    ...
    pub fn serialize_compressed(&self, batch: &Batch, format: SerializationFormat) -> Result<Vec<u8>, SerializationError> {
