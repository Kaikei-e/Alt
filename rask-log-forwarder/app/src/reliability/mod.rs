...
pub use retry::{RetryManager, RetryConfig, RetryStrategy, RetryError};
pub use disk::{DiskFallback, DiskConfig, DiskError};
pub use metrics::{MetricsCollector, PrometheusExporter, MetricsConfig, MetricsSnapshot, MetricsError};
pub use health::{HealthMonitor, HealthConfig, HealthStatus, ComponentHealth, HealthReport};
...
impl ReliabilityManager {
    ...
    pub async fn send_batch_with_reliability(&self, batch: Batch) -> Result<(), TransmissionError> {
        ...
        // Record metrics
        {
            ...
            match &result {
                Ok(_) => {
                    metrics.record_batch_sent(entry_count, true, std::time::Duration::from_millis(0)).await;
                    self.health_monitor.record_health_check("transmission", true).await;
                }
                Err(_) => {
                    metrics.record_batch_sent(entry_count, false, std::time::Duration::from_millis(0)).await;
                    self.health_monitor.record_health_check("transmission", false).await;
                }
            }
        }
    ...
    async fn attempt_transmission_with_retries(&self, batch: Batch) -> Result<(), TransmissionError> {
        let batch_id = batch.id().to_string();
        ...
        loop {
            ...
            match self.log_sender.send_batch(batch.clone()).await {
                Ok(result) => {
                    if result.success {
                        tracing::info!("Successfully transmitted batch {} on attempt {}", 
                                     batch_id, self.get_attempt_count(&batch_id).await);
                        return Ok(());
                    } else {
                        tracing::warn!("Transmission failed with HTTP {}: {}", 
                                     result.status_code, batch_id);
                    }
                }
                Err(e) => {
            ...
            // Increment retry attempt
            {
                ...
                if !retry_manager.should_give_up(&batch_id) {
                    ...
                    tracing::info!("Retrying batch {} in {:?} (attempt {})", 
                                 batch_id, delay, attempt_count + 1);
    ...
    async fn store_to_disk_fallback(&self, batch: Batch) -> Result<(), TransmissionError> {
        ...
        tracing::warn!("Storing batch {} to disk fallback after failed retries", batch_id);
        
        {
            ...
            if let Err(e) = disk_fallback.store_batch(batch).await {
                ...
                self.health_monitor.update_component_health("disk_fallback", 
                    ComponentHealth::Unhealthy(format!("Disk storage failed: {}", e))).await;
                return Err(TransmissionError::ClientError(
                    crate::sender::ClientError::InvalidConfiguration(
                        format!("Disk fallback failed: {}", e)
                    )
                ));
            }
        ...
        self.health_monitor.update_component_health("disk_fallback", ComponentHealth::Healthy).await;
        Ok(())
    }
    ...
    pub async fn start_background_tasks(&self) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                interval.tick().await;
                health_monitor.cleanup_stale_components(std::time::Duration::from_secs(1800)).await; // 30 minutes
            }
        });
    }
