...
impl DiskFallback {
    ...
    pub async fn store_batch(&mut self, batch: Batch) -> Result<(), DiskError> {
        ...
        // Serialize batch
        let serialized = bincode::serialize(&stored_batch)?;
    ...
    pub async fn retrieve_batch(&self, batch_id: &str) -> Result<Batch, DiskError> {
        ...
        // Deserialize
        let stored_batch: StoredBatch = bincode::deserialize(&deserialized_data)?;
    ...
    async fn load_stored_batch(&self, batch_id: &str) -> Result<StoredBatch, DiskError> {
        ...
        bincode::deserialize(&deserialized_data).map_err(DiskError::SerializationError)
    }
