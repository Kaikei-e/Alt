...
mod tests {
    ...
    fn test_pattern_index_constants() {
        ...
        assert_eq!(
            get_pattern_name(pattern_index::DOCKER_NATIVE_TIMESTAMP),
            Some("docker_native_timestamp")
        );
        assert_eq!(
            get_pattern_name(pattern_index::ISO_TIMESTAMP_FALLBACK),
            Some("iso_timestamp_fallback")
        );
        assert_eq!(
            get_pattern_name(pattern_index::NGINX_ACCESS_FALLBACK),
            Some("nginx_access_fallback")
        );
        assert_eq!(
            get_pattern_name(pattern_index::POSTGRES_LOG),
            Some("postgres_log")
        );
    }
    ...
    fn test_pattern_by_name_access() {
        ...
        assert!(
            VALIDATED_PATTERNS
                .get_by_name("docker_native_timestamp")
                .is_ok()
        );
        assert!(
            VALIDATED_PATTERNS
                .get_by_name("nginx_access_fallback")
                .is_ok()
        );
        ...
        assert!(
            VALIDATED_PATTERNS
                .get_by_name("nonexistent_pattern")
                .is_err()
        );
    }
}
