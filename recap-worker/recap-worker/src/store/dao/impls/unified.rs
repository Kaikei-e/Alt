...
// CardsDao implementation
impl crate::store::dao::cards::CardsDao for UnifiedDao {
    async fn insert_card_snapshot(
        &self,
        snapshot: &crate::store::dao::cards::RecapCardSnapshot,
    ) -> Result<()> {
        crate::store::dao::cards::CardsDaoOps::insert_snapshot(&self.pool, snapshot).await
    }

    async fn get_card_snapshot(
        &self,
        job_id: Uuid,
    ) -> Result<Option<crate::store::dao::cards::RecapCardSnapshot>> {
        crate::store::dao::cards::CardsDaoOps::get_snapshot(&self.pool, job_id).await
    }

    async fn insert_card_candidates(
        &self,
        candidates: &[crate::store::dao::cards::RecapCardCandidate],
    ) -> Result<()> {
        crate::store::dao::cards::CardsDaoOps::insert_candidates(&self.pool, candidates).await
    }

    async fn get_card_candidates(
        &self,
        job_id: Uuid,
    ) -> Result<Vec<crate::store::dao::cards::RecapCardCandidate>> {
        crate::store::dao::cards::CardsDaoOps::get_candidates_for_job(&self.pool, job_id).await
    }
    ...
    async fn get_candidates_for_window(
        ...
        crate::store::dao::cards::CardsDaoOps::get_candidates_for_window(&self.pool, window_id)
            .await
    }

    async fn insert_cards(&self, cards: &[crate::store::dao::cards::RecapCard]) -> Result<()> {
        crate::store::dao::cards::CardsDaoOps::insert_cards(&self.pool, cards).await
    }

    async fn get_cards_for_job(
        &self,
        job_id: Uuid,
    ) -> Result<Vec<crate::store::dao::cards::RecapCard>> {
        crate::store::dao::cards::CardsDaoOps::get_cards_for_job(&self.pool, job_id).await
    }

    async fn insert_job_stats(
        &self,
        stats: &crate::store::dao::cards::RecapCardJobStats,
    ) -> Result<()> {
        crate::store::dao::cards::CardsDaoOps::insert_job_stats(&self.pool, stats).await
    }

    async fn get_job_stats(
        &self,
        job_id: Uuid,
    ) -> Result<Option<crate::store::dao::cards::RecapCardJobStats>> {
        crate::store::dao::cards::CardsDaoOps::get_job_stats(&self.pool, job_id).await
    }

    async fn insert_eval_window(
        &self,
        window: &crate::store::dao::cards::RecapEvalWindow,
    ) -> Result<()> {
        crate::store::dao::cards::CardsDaoOps::insert_eval_window(&self.pool, window).await
    }

    async fn get_eval_window(
        &self,
        window_id: Uuid,
    ) -> Result<Option<crate::store::dao::cards::RecapEvalWindow>> {
        crate::store::dao::cards::CardsDaoOps::get_eval_window(&self.pool, window_id).await
    }

    async fn list_eval_windows(
        &self,
    ) -> Result<Vec<crate::store::dao::cards::RecapEvalWindowSummary>> {
        crate::store::dao::cards::CardsDaoOps::list_eval_windows(&self.pool).await
    }

    async fn insert_story_judgment(
        &self,
        judgment: &crate::store::dao::cards::RecapStoryJudgment,
    ) -> Result<()> {
        crate::store::dao::cards::CardsDaoOps::insert_story_judgment(&self.pool, judgment).await
    }

    async fn latest_judgments_for_window(
        &self,
        window_id: Uuid,
    ) -> Result<Vec<crate::store::dao::cards::RecapStoryJudgment>> {
        crate::store::dao::cards::CardsDaoOps::latest_judgments_for_window(&self.pool, window_id)
            .await
    }

    async fn insert_card_rating(
        &self,
        rating: &crate::store::dao::cards::RecapCardRating,
    ) -> Result<()> {
        crate::store::dao::cards::CardsDaoOps::insert_card_rating(&self.pool, rating).await
    }

    async fn latest_rating_per_card(
        &self,
        job_id: Uuid,
    ) -> Result<Vec<crate::store::dao::cards::RecapCardRating>> {
        crate::store::dao::cards::CardsDaoOps::latest_rating_per_card(&self.pool, job_id).await
    }

    async fn list_cards_jobs(
        &self,
        limit: i64,
    ) -> Result<Vec<crate::store::dao::cards::CardsJobSummary>> {
        crate::store::dao::cards::CardsDaoOps::list_cards_jobs(&self.pool, limit).await
    }
}
