...
INFRASTRUCTURE_CHARTS=(
  ...
  postgres auth-postgres kratos-postgres clickhouse meilisearch nginx nginx-external
)
...
get_image_overrides(){
  ...
  case "$chart" in
    alt-backend|auth-service|pre-processor|search-indexer|tag-generator|news-creator|rask-log-aggregator|alt-frontend)
      ...
      ;;
    postgres|clickhouse|meilisearch)
      echo "--set image.tag=${TAG_BASE}"
      ;;
    *)
      # 共通Chartやその他はイメージオーバーライドなし
      ;;
  esac
}
