...
func (u *HealthCheckUsecase) performSingleNamespaceHealthCheck(ctx context.Context, chart domain.Chart, namespace string, options *domain.DeploymentOptions) error {
	...
	if healthCheckType, exists := chart.Annotations["deploy-cli/health-check"]; exists {
		u.logger.InfoWithContext("using annotation-based health check", map[string]interface{}{
			"chart_name":        chart.Name,
		...
		case "none":
			u.logger.InfoWithContext("skipping health check per annotation", map[string]interface{}{
				"chart_name": chart.Name,
			})
			return nil
		case "secret":
		...
		default:
			u.logger.WarnWithContext("unknown health check annotation value, falling back to automatic detection", map[string]interface{}{
				"chart_name":        chart.Name,
				"health_check_type": healthCheckType,
			})
			// Continue to automatic detection below instead of using default
		}
	}
...
func (u *HealthCheckUsecase) verifySecretChart(ctx context.Context, chartName, namespace string) error {
	...
	// For secret-only charts, verify that required secrets exist and are valid
	if err := u.healthChecker.WaitForSecretsReady(ctx, chartName, namespace); err != nil {
...
func (u *HealthCheckUsecase) isSecretOnlyChart(chartName string) bool {
	secretCharts := []string{
		"common-secrets", // Creates K8s secrets
		"common-ssl",     // Creates SSL certificates and secrets
		// Note: "common-config" removed - it's a configuration chart (namespaces, network policies, storage classes)
		// and should use the annotation-based health check strategy instead
	}
