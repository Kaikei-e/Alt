...
Provides mocks for ML models and NLTK dependencies to enable testing without external downloads.
"""
...
def mock_ml_models():
    ...
    mock_keybert.extract_keywords.return_value = [("machine learning", 0.8), ("ai", 0.6)]
    ...
    with patch('tag_extractor.extract.SentenceTransformer', return_value=mock_sentence_transformer), \
         patch('tag_extractor.extract.KeyBERT', return_value=mock_keybert), \
         patch('tag_extractor.extract.Tagger', return_value=mock_tagger), \
         patch('tag_extractor.model_manager.SentenceTransformer', return_value=mock_sentence_transformer), \
...
def mock_nltk_data():
    ...
    mock_stopwords = {
        'english': ['the', 'a', 'an', 'is', 'are', 'was', 'were', 'be', 'been', 'being',
                   ...
                   'at', 'by', 'with', 'from', 'as', 'but', 'or', 'and', 'not', 'this',
                   'that', 'these', 'those']
    }
    ...
    with patch('nltk.corpus.stopwords', mock_stopwords_corpus), \
