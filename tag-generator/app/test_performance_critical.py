...
class TestCriticalPerformanceRequirements:
    ...
    def test_model_singleton_performance(self):
        ...
        with patch.object(model_manager, '_load_models', wraps=model_manager._load_models) as mock_load_models:
            ...
            assert mock_load_models.call_count <= 1, f"Models loaded {mock_load_models.call_count} times, should be ≤ 1 (shared singleton)"
    ...
    def test_connection_pool_performance(self):
        ...
        assert mock_connect.call_count <= 3, f"Created {mock_connect.call_count} connections, should use connection pooling"
    ...
    def test_batch_processing_efficiency(self):
        ...
        assert speedup >= 5.0, f"Batch speedup {speedup:.1f}x should be ≥ 5x (got individual: {individual_time:.3f}s, batch: {batch_time:.3f}s)"
    ...
    def test_memory_growth_control(self):
        ...
        for batch in range(10):  # 10 batches
            for i in range(50):  # 50 articles per batch
                ...
                tags = extractor.extract_tags(f"Title {batch}-{i}", content)
        ...
        assert memory_growth < 300, f"Memory grew by {memory_growth:.1f}MB processing 500 articles, should be < 300MB (improved from previous 400MB+)"
    ...
    def test_concurrent_extraction_scalability(self):
        ...
        assert speedup >= expected_min_speedup, f"Concurrent speedup {speedup:.1f}x should be ≥ {expected_min_speedup:.1f}x"
    ...
    def test_database_transaction_efficiency(self):
        ...
        total_db_calls = (mock_cursor.execute.call_count + 
