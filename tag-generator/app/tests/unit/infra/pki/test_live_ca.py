...
from tag_generator.infra.pki.native_issuer import NativeStepCAIssuer, _verify_chain
...
LIVE_CONTAINER = "alt-pki-taggen-itest"
LIVE_NETWORK = "alt-pki-taggen-itest-net"
LIVE_HOST_PORT = "19011"
LIVE_CA_PASSWORD = "itest-only"
LIVE_JWK_PASSWORD = "itest-jwk-tag-generator"
LIVE_PROVISIONER = "pki-agent-tag-generator"
LIVE_SUBJECT = "tag-generator"
...
async def test_native_issuer_disposable_live_ca(tmp_path: Path) -> None:
    ...
    try:
        ...
        _add_provisioner()
        password_file = tmp_path / "pki-agent-tag-generator-jwk"
...
printf '%s\\n' {LIVE_JWK_PASSWORD!r} > /tmp/pki-agent-tag-generator-jwk
chmod 400 /tmp/pki-agent-tag-generator-jwk
step ca provisioner add {LIVE_PROVISIONER!r} --type JWK --create \\
  --password-file /tmp/pki-agent-tag-generator-jwk \\
