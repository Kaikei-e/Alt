...
class TestTagExtractorPerformance:
    ...
    def test_should_process_multiple_articles_under_time_limit(self):
        ...
        test_articles = [
            ("Tech News", "Machine learning and artificial intelligence are transforming the industry"),
            ("Sports Update", "The championship game delivered an exciting finish for all fans"),
            ("Weather Report", "Sunny skies and mild temperatures expected throughout the week"),
            ("Health Tips", "Regular exercise and balanced nutrition contribute to overall wellness"),
            ("Travel Guide", "Exploring beautiful destinations and discovering local culture experiences"),
            ("Business News", "Stock market trends and economic indicators show positive growth"),
            ("Science Discovery", "Researchers made breakthrough findings in quantum physics experiments"),
            ("Food Review", "The restaurant offers authentic cuisine with fresh ingredients"),
            ("Technology Update", "New software features enhance user experience and productivity"),
            ("Education News", "Students demonstrate improved learning outcomes with innovative teaching methods")
        ]
    ...
    def test_should_manage_memory_efficiently(self):
        ...
        for batch in range(5):
            for i in range(20):
                tags = extractor.extract_tags(f"Title {i}", f"Content for article {i} in batch {batch}")
...
class TestBatchProcessingPerformance:
    ...
    def test_should_process_batch_faster_than_individual(self):
        ...
        assert batch_time < individual_time * 0.7, f"Batch time {batch_time:.3f}s should be faster than individual {individual_time:.3f}s"
...
class TestServicePerformance:
    ...
    def test_should_reuse_database_connections(self, mock_connect):
        ...
        with patch.object(TagGeneratorService, '_process_article_batch') as mock_process:
            mock_process.return_value = {"total_processed": 5, "successful": 5, "failed": 0}
    ...
    def test_should_process_cycles_with_consistent_performance(self):
        ...
        with patch.object(service, '_create_database_connection') as mock_conn, \
             patch.object(service, '_process_article_batch') as mock_process:
            
            mock_conn.return_value = Mock()
            mock_process.return_value = {"total_processed": 10, "successful": 10, "failed": 0}
        ...
        assert max_deviation < avg_time * 0.5, f"Performance variation {max_deviation:.3f}s is too high for avg {avg_time:.3f}s"
...
class TestMemoryManagement:
    ...
    def test_should_cleanup_resources_after_processing(self):
        ...
        extractor = TagExtractor()
        for i in range(100):
            tags = extractor.extract_tags(f"Title {i}", f"Content for article number {i} with some text")
