...
def main():
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        if result.is_valid:
            print(f"   ✅ VALID - Input accepted and sanitized")
            if result.sanitized_input:
                print(f"   📝 Sanitized title: {result.sanitized_input.title[:50]}{'...' if len(result.sanitized_input.title) > 50 else ''}")
                print(f"   📝 Original length: {result.sanitized_input.original_length}, Sanitized length: {result.sanitized_input.sanitized_length}")
        else:
            print(f"   ❌ BLOCKED - Input rejected")
