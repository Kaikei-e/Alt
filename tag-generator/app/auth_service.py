...
class AuthenticatedTagGeneratorService:
    ...
    def __init__(self):
        ...
        logger.info("Authenticated tag generator service initialized",
    ...
    async def generate_personalized_tags(
        ...
        logger.info("Generating personalized tags",
        ...
        try:
            ...
            user_preferences = await self._get_user_tag_preferences(
                user_context.tenant_id, 
                user_context.user_id
            )
            
            # Generate tags using ML model with user preferences
            tags = await self._generate_tags_with_preferences(
                request,
                user_preferences
            )
            
            # Save generated tags to user-specific storage
            await self._save_user_tags(
                user_context.tenant_id,
                user_context.user_id,
                request.article_id,
                tags
            ...
            logger.info("Personalized tags generated successfully",
                       user_id=user_context.user_id,
                       tenant_id=user_context.tenant_id,
                       article_id=request.article_id,
                       tag_count=len(tags))
            ...
            logger.error("Failed to generate personalized tags",
                        user_id=user_context.user_id,
                        tenant_id=user_context.tenant_id,
                        article_id=request.article_id,
                        error=str(e))
    ...
    async def _get_user_tag_preferences(
        ...
        logger.debug("Retrieving user tag preferences",
                    tenant_id=tenant_id,
                    user_id=user_id)
...
async def generate_tags_endpoint(
    ...
    try:
        ...
        logger.error("Tag generation endpoint failed",
                    user_id=user_context.user_id,
                    article_id=request.article_id,
                    error=str(e))
...
async def get_user_preferences(user_context: UserContext) -> Dict[str, Any]:
    """Get user's tag generation preferences."""
    preferences = await tag_service._get_user_tag_preferences(
        user_context.tenant_id,
        user_context.user_id
    )
