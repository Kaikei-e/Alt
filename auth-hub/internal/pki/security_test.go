...
func TestLoadConfig_ExactProvisionerAndPasswordBasename(t *testing.T) {
	...
	t.Setenv("STEP_CA_PROVISIONER", "pki-agent-auth-hub-extra")
	t.Setenv("STEP_CA_PROVISIONER_PASSWORD_FILE", "/run/secrets/pki-agent-auth-hub-extra-jwk")
...
func TestLoadConfig_TempDirAllowedWhenBasenameMatches(t *testing.T) {
	t.Setenv("PKI_ENROLLMENT", ModeEnabled)
	t.Setenv("CERT_SUBJECT", "auth-hub")
	path := filepath.Join(t.TempDir(), "pki-agent-auth-hub-jwk")
...
func TestNativeStepCAIssuer_RejectsWrongP2CBeforeDecrypt(t *testing.T) {
	ca := newFakeStepCA(t, "pki-agent-auth-hub", []byte("pw-p2c"))
...
func TestNativeStepCAIssuer_RequiresDualEKU(t *testing.T) {
	ca := newFakeStepCA(t, "pki-agent-auth-hub", []byte("pw-eku"))
...
func TestNativeStepCAIssuer_CARootSymlinkRejected(t *testing.T) {
	ca := newFakeStepCA(t, "pki-agent-auth-hub", []byte("pw-rootlink"))
	iss := newNativeIssuer(t, ca)
	dir := t.TempDir()
...
func TestCertFile_CertRenameFailureRollsBackKey(t *testing.T) {
	...
	cf := &CertFile{CertPath: filepath.Join(dir, "svc-cert.pem"), KeyPath: filepath.Join(dir, "svc-key.pem")}
	nb := time.Now().Add(-time.Minute)
	oldCert, oldKey := newSelfSignedPEM(t, "auth-hub", nb, nb.Add(time.Hour))
	if err := cf.Write(context.Background(), oldCert, oldKey); err != nil {
		t.Fatal(err)
	}
	newCert, newKey := newSelfSignedPEM(t, "auth-hub", nb, nb.Add(2*time.Hour))
...
func TestCertFile_CrashBetweenRenames_LoadCorruptAndTickReissues(t *testing.T) {
	...
	newCert, newKey := newSelfSignedPEM(t, "auth-hub", nb, nb.Add(48*time.Hour))
...
func TestTick_CrashKeepsTLSUtilLastGood(t *testing.T) {
	...
	first, err := cfg.GetCertificate(&tls.ClientHelloInfo{ServerName: "auth-hub"})
	if err != nil {
		t.Fatal(err)
	}
	newCert, newKey := newSelfSignedPEM(t, "auth-hub", nb, nb.Add(2*time.Hour))
	...
	files.afterKeyRename = nil
	second, err := cfg.GetCertificate(&tls.ClientHelloInfo{ServerName: "auth-hub"})
	if err != nil {
		t.Fatal(err)
	}
