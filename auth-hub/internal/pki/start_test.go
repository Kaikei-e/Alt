...
func TestStart_DisabledLogs(t *testing.T) {
	...
	h, err := Start(context.Background(), log, "auth-hub")
	if err != nil {
		t.Fatal(err)
	}
	if h != nil {
...
func TestStart_EnabledDoesNotRequireStepBinary(t *testing.T) {
	...
	_, err := Start(context.Background(), slog.Default(), "auth-hub")
	if err == nil {
...
func TestStartEnabled_PrivateRegistryNotDefault(t *testing.T) {
	...
	cfg := &Config{
		Mode:            ModeEnabled,
		Subject:         "auth-hub-private-reg",
		SANs:            []string{"auth-hub-private-reg"},
		CertPath:        filepath.Join(dir, "svc-cert.pem"),
		KeyPath:         filepath.Join(dir, "svc-key.pem"),
		Provisioner:     "pki-agent-auth-hub-private-reg",
		PasswordFile:    "/run/secrets/pki-agent-auth-hub-private-reg-jwk",
		RenewAtFraction: 0.66,
		TickInterval:    time.Hour,
		RetryAttempts:   1,
		RetryBackoff:    time.Millisecond,
	...
	needle := `pki_enrollment_healthy{subject="auth-hub-private-reg"} 1`
	...
	if strings.Contains(def.Body.String(), `subject="auth-hub-private-reg"`) {
...
func TestStart_EnabledSharedSecretRejected(t *testing.T) {
	...
	if _, err := Start(context.Background(), slog.Default(), "auth-hub"); err == nil {
