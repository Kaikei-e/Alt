"""Tests for peer-identity middleware (recap-subworker)."""
...
from recap_subworker.app.infra.inbound_tls import forget_tls_peer, remember_tls_peer
...
# 127.0.0.1:8002, so a loopback transport peer is the sidecar. Anything else
# reached the published plaintext port directly.
SIDECAR = ("127.0.0.1", 44444)
DIRECT = ("172.18.0.9", 44444)

# These cases drive TestClient without the context manager: tests/conftest.py
# patches __enter__ to install FastAPI dependency overrides, which a bare
# Starlette app has no attribute for.
...
def test_tls_peer_cn_wins_over_spoofed_header(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    try:
        app = _build_app(monkeypatch, allowed=["recap-worker"], strict=True)
        http = TestClient(app, client=client)
