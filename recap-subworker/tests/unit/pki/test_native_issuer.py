...
from recap_subworker.app.infra.pki.config import PkiError, SharedProvisionerError
...
class FakeStepCA:
            ...
            def do_GET(self) -> None:
                ...
                if path == "/provisioners":
                    if ca.endless_provisioners:
                        body = json.dumps({"provisioners": [], "nextCursor": "more"}).encode()
                    else:
    ...
    def _record(self, handler: BaseHTTPRequestHandler) -> None:
        ...
        with self.mu:
            self.seen.append(RecordedReq(handler.command, handler.path.split("?", 1)[0], body))
    ...
    def verify_ott(self, ott: str) -> dict[str, Any]:
        ...
        exp = claims.get("exp")
        if self.reject_expired and exp is not None and datetime.now(UTC).timestamp() > float(exp):
    ...
    def _sign_csr(self, csr: x509.CertificateSigningRequest, mutate: Any) -> x509.Certificate:
        try:
        ...
        if mutate is not None:
            ...
            builder = (
                x509.CertificateBuilder()
                .subject_name(
                    x509.Name([x509.NameAttribute(NameOID.COMMON_NAME, tmpl.common_name)])
                )
                .issuer_name(self.ca_cert.subject)
                .public_key(csr.public_key())
                .serial_number(x509.random_serial_number())
                ...
                .not_valid_after(datetime.now(UTC) + timedelta(hours=1))
                .add_extension(
                    x509.SubjectAlternativeName([x509.DNSName(n) for n in tmpl.dns_names]),
                    critical=False,
                )
                .add_extension(
                    x509.ExtendedKeyUsage(
    ...
    def _write_sign_response(self, handler: BaseHTTPRequestHandler, leaf: x509.Certificate) -> None:
        ...
        body = json.dumps({"crt": leaf_pem, "ca": ca_pem, "certChain": [leaf_pem, ca_pem]}).encode()
        handler.send_response(201)
        handler.send_header("Content-Type", "application/json")
        handler.end_headers()
...
def _new_issuer(ca: FakeStepCA, tmp_path: Path) -> NativeStepCAIssuer:
    ...
    pw = _write_password_file(
        tmp_path, "pki-agent-recap-subworker-jwk", ca.password.decode(), 0o400
    )
...
def test_reused_ott_rejected_by_ca(tmp_path: Path) -> None:
    ...
    try:
        ...
        conn.request("POST", "/sign", body=first.body, headers={"Content-Type": "application/json"})
...
def test_expired_ott_rejected_by_ca(tmp_path: Path) -> None:
    ...
    try:
        ...
        from recap_subworker.app.infra.pki.native_issuer import _create_csr
        ...
        assert host is not None
        conn = http.client.HTTPSConnection(host, parsed.port, context=ctx, timeout=5)
        conn.request("POST", "/sign", body=body, headers={"Content-Type": "application/json"})
        resp = conn.getresponse()
        assert resp.status >= 400
        conn.close()
    finally:
...
def test_wrong_ca_root(tmp_path: Path) -> None:
    ...
    try:
        ...
        with pytest.raises((OSError, RuntimeError, ssl.SSLError, ValueError)) as excinfo:
            iss.issue(Ctx(), SUBJECT, [SUBJECT])
...
def test_timeout(tmp_path: Path) -> None:
    ...
    try:
        ...
        with pytest.raises((TimeoutError, OSError, RuntimeError, CancelledError, PkiError)):
...
def test_decrypt_provisioner_jwk_password_and_bytes() -> None:
    ...
    key["alg"] = "ES256"
    password = b"itest-jwk-recap-subworker"
    got = decrypt_provisioner_jwk(_encrypt_jwk(key, password), password)
    assert json.loads(got.export_private())["d"] == json.loads(key.export_private())["d"]
