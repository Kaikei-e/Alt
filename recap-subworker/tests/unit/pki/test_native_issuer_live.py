...
from recap_subworker.app.infra.pki.ctx import Ctx
...
LIVE_CA_CONTAINER = "alt-pki-native-itest-recap-subworker"
LIVE_CA_NETWORK = "alt-pki-native-itest-recap-subworker-net"
LIVE_CA_HOST_PORT = "19011"
LIVE_CA_PASSWORD = "itest-only"
LIVE_JWK_PASSWORD = "itest-jwk-recap-subworker"
LIVE_PROVISIONER = "pki-agent-recap-subworker"
LIVE_SUBJECT = "recap-subworker"
...
def test_native_step_ca_issuer_disposable_live_ca(tmp_path: Path) -> None:
    ...
    try:
        subprocess.run(["docker", "rm", "-f", LIVE_CA_CONTAINER], check=False, capture_output=True)
        subprocess.run(
            ["docker", "network", "rm", LIVE_CA_NETWORK], check=False, capture_output=True
        )
        ...
        password_file = tmp_path / "pki-agent-recap-subworker-jwk"
        ...
        rekey_cert, rekey_key = iss.rekey(ctx, cert_pem, key_pem, LIVE_SUBJECT, [LIVE_SUBJECT])
        ...
        leaf3 = _must_live_leaf(after_rekey, Path(files.key_path).read_bytes(), root_file)
...
def _wait_live_ca_root(dest: Path, deadline: float) -> None:
    ...
    while time.monotonic() < deadline:
        probe = subprocess.run(
            ["docker", "exec", LIVE_CA_CONTAINER, "test", "-f", "/home/step/certs/root_ca.crt"],
            check=False,
            capture_output=True,
            text=True,
        )
        if probe.returncode == 0:
            subprocess.run(
                ["docker", "cp", f"{LIVE_CA_CONTAINER}:/home/step/certs/root_ca.crt", str(dest)],
                check=True,
                capture_output=True,
            )
            return
        ...
        time.sleep(0.2)
    logs = subprocess.run(
        ["docker", "logs", LIVE_CA_CONTAINER], check=False, capture_output=True, text=True
    )
...
printf '%s\\n' {LIVE_JWK_PASSWORD!r} > /tmp/pki-agent-recap-subworker-jwk
chmod 400 /tmp/pki-agent-recap-subworker-jwk
step ca provisioner add {LIVE_PROVISIONER!r} --type JWK --create \\
  --password-file /tmp/pki-agent-recap-subworker-jwk \\
    ...
    while time.monotonic() < deadline:
        ...
        try:
            ...
            for path in ("/provisioners?limit=100", "/1.0/provisioners?limit=100"):
                conn = http.client.HTTPSConnection(host, parsed.port, context=ctx, timeout=2)
                try:
            ...
            last = str(exc)
        time.sleep(0.25)
    pytest.fail(f"provisioner {LIVE_PROVISIONER} missing encryptedKey on HTTP API: {last}")
...
def _must_live_leaf(cert_pem: bytes, key_pem: bytes, root_file: Path) -> x509.Certificate:
    ...
    from recap_subworker.app.infra.pki.native_issuer import _assert_sans, _verify_chain
