...
from tests.unit.pki.test_certfile import new_self_signed_pem
...
def _mgr(tmp_path: Path, issuer: FakeIssuer, now: datetime) -> tuple[Manager, CertFile, RecObs]:
...
def test_tick_mismatched_pair_never_classifies_fresh(tmp_path: Path) -> None:
    nb = datetime(2026, 8, 18, tzinfo=UTC)
    iss = FakeIssuer(err=RuntimeError("no-issue"), not_before=nb, lifetime=timedelta(hours=24))
...
def test_tick_near_expiry_uses_rekey_when_available(tmp_path: Path) -> None:
    nb = datetime(2026, 8, 18, tzinfo=UTC)
    iss = RecordingRekeyIssuer(not_before=nb + timedelta(hours=16), lifetime=timedelta(hours=24))
    mgr, files, obs = _mgr(tmp_path, iss, nb + timedelta(hours=16))
    cert, key = new_self_signed_pem(SUBJECT, nb, nb + timedelta(hours=24))
    files.write(cert, key)
    mgr.tick(Ctx())
...
def test_tick_expired_does_not_rekey(tmp_path: Path) -> None:
    nb = datetime(2026, 8, 18, tzinfo=UTC)
    iss = RecordingRekeyIssuer(not_before=nb + timedelta(hours=25), lifetime=timedelta(hours=24))
    mgr, files, obs = _mgr(tmp_path, iss, nb + timedelta(hours=25))
    cert, key = new_self_signed_pem(SUBJECT, nb, nb + timedelta(hours=24))
    files.write(cert, key)
    mgr.tick(Ctx())
