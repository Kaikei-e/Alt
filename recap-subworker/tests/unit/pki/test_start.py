...
from recap_subworker.app.infra.pki.config import MODE_DISABLED, MODE_ENABLED, Config
from recap_subworker.app.infra.pki.start import start, start_with
...
def test_start_enabled_does_not_require_step_binary(
    ...
    monkeypatch.setenv(
        "STEP_CA_PROVISIONER_PASSWORD_FILE", str(tmp_path / "pki-agent-recap-subworker-jwk")
    )
    with pytest.raises((OSError, RuntimeError, ValueError)) as excinfo:
        start("recap-subworker")
