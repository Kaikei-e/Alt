...
def test_inbound_handshake_shows_new_not_after_after_atomic_replace(tmp_path: Path) -> None:
    ...
    cert, key, ca = _write_leaf(tmp_path, "recap-subworker", days=1)
    ...
    try:
        ...
        new_cert, new_key, _new_ca = _write_leaf(tmp_path, "recap-subworker", days=30)
        ...
        assert second != first, (
            "inbound handshake must present the rotated NotAfter without restarting the process"
        )
...
def test_inbound_listener_has_no_response_header_timeout() -> None:
    ...
    assert timeouts.response_header_timeout is None, (
        ...
        "PROXY_RESPONSE_HEADER_TIMEOUT; long clustering RPCs would 504"
    )
...
def test_slow_first_byte_is_not_cut_by_header_timeout(tmp_path: Path) -> None:
    ...
    pki-agent's default ResponseHeaderTimeout is 15s; recap's sidecar raises
    that to 1560s. The in-process listener has no such cutoff, so a request
    that waits 2s before writing headers (far above a mistaken 1s default,
    cheap enough for CI) must return 200.
    """
    cert, key, ca = _write_leaf(tmp_path, "recap-subworker", days=1)
    ctx = build_inbound_ssl_context(str(cert), str(key), str(ca))
