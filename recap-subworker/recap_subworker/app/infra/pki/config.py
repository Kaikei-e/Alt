...
class UnsupportedIssuerAlgorithm(PkiError):  # noqa: N818
...
def _require_inbound_mtls(env: Mapping[str, str]) -> None:
    ...
    if raw not in {"true", "1", "on"}:
        raise PKIConfigError(
            f"pki: PKI_ENROLLMENT=enabled requires INBOUND_MTLS=true (got {env['INBOUND_MTLS']!r})"
        )
...
def _validate(cfg: Config) -> None:
    ...
    if str(Path(cleaned).parent) == "/run/secrets" and cleaned != provisioner_password_file(
        cfg.subject
    ):
        raise PkiError(
