...
from recap_subworker.app.infra.pki.certfile import CertFile, CertNotFoundError, CertParseError
...
from recap_subworker.app.infra.pki.filesafe import MAX_ROOT_PEM_BYTES, read_regular_no_follow
from recap_subworker.app.infra.pki.state import State, classify_remaining
...
class Manager:
    ...
    def enroll(self, ctx: Ctx) -> None:
        ...
        backoff = self.cfg.retry_backoff if self.cfg.retry_backoff > 0 else DEFAULT_BACKOFF_SECONDS
        ...
        raise RuntimeError(f"pki: enroll failed after {attempts} attempts: {last}") from last
    ...
    def tick(self, ctx: Ctx) -> State:
        ...
        now = self.now()
        state = classify_remaining(cert.not_before, cert.not_after, now, self.cfg.renew_at_fraction)
    ...
    def _issue(self, ctx: Ctx, reason: str) -> State:
        ...
        try:
            cert_pem, key_pem = self.issuer.issue(ctx, self.cfg.subject, list(self.cfg.sans))
    ...
    def _rekey(self, ctx: Ctx) -> State:
        ...
        try:
            new_cert, new_key = rekey(ctx, cert_pem, key_pem, self.cfg.subject, list(self.cfg.sans))
        except CancelledError:
            self.observer.on_renewed(False)
            raise
        except Exception as exc:
    ...
    def run(self, ctx: Ctx) -> None:
        interval = self.cfg.tick_interval if self.cfg.tick_interval > 0 else DEFAULT_TICK_SECONDS
        ...
        logger.info("pki_enrollment_stopped subject=%s error=canceled", self.cfg.subject)
        raise CancelledError("canceled")
