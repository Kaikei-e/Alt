...
--
-- This table lives in recap-db, not in alt-db, and that placement is
...
-- business state change share one local commit. Recap job state lives here,
...
--
-- A relay in recap-worker forwards rows to alt-data-hub over mTLS and marks
-- them forwarded. Delivery to the device is tracked separately, in alt-db's
-- push_deliveries.
CREATE TABLE notification_outbox (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    -- Derived from the business fact (e.g. 'recap:<job_id>'), never generated
...
COMMENT ON TABLE notification_outbox IS 'Transactional outbox for push notifications (forwarded to alt-data-hub by the recap-worker relay)';
