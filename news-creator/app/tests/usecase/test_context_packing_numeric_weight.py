...
def test_numeric_fact_sentence_outranks_plain():
    ...
    score_numeric = usecase._score_sentence_for_packing(numeric, cluster_sentences=[numeric])
...
def test_percent_only_sentence_still_boosted():
    ...
    assert usecase._score_sentence_for_packing(pct, [pct]) > usecase._score_sentence_for_packing(
        plain, [plain]
    )
...
def test_date_only_sentence_still_boosted():
    ...
    assert usecase._score_sentence_for_packing(dated, [dated]) > usecase._score_sentence_for_packing(
        plain, [plain]
    )
