...
generative AI development""",
    ...
    assert call_args.kwargs.get("model") == "gemma3-4b-16k"
...
async def test_expand_query_uses_correct_model():
    """Test that query expansion uses gemma3-4b-16k model specifically."""
    config = Mock()
    llm_provider = AsyncMock()
    llm_provider.generate.return_value = LLMGenerateResponse(
    ...
    assert usecase.EXPANSION_MODEL == "gemma3-4b-16k"
    call_args = llm_provider.generate.call_args
    assert call_args.kwargs.get("model") == "gemma3-4b-16k"
