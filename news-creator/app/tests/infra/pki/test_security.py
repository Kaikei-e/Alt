...
from news_creator.infra.pki.filesafe import MAX_PASSWORD_BYTES
from news_creator.infra.pki.manager import Manager
from news_creator.infra.pki.metrics import PromObserver
...
from tests.infra.pki.helpers import self_signed_pem, write_password_file
from tests.infra.pki.test_manager import FakeIssuer
from tests.infra.pki.test_native_issuer import SUBJECT, FakeStepCA, _new_issuer

NB = datetime(2026, 8, 18, tzinfo=UTC)


def _enabled(monkeypatch: pytest.MonkeyPatch, subject: str = "news-creator") -> None:
...
def test_enrollment_file_missing_fails(
...
def test_enrollment_file_empty_fails(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    path = tmp_path / "empty"
    path.write_text(" \n", encoding="utf-8")
...
def test_enrollment_file_oversize_fails(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    path = tmp_path / "huge"
...
def test_exact_provisioner_and_password_basename(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _enabled(monkeypatch, "acolyte")
    monkeypatch.setenv("STEP_CA_PROVISIONER", "pki-agent-news-creator")
    monkeypatch.setenv(
...
def test_rejects_wrong_password_basename(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    _enabled(monkeypatch)
    monkeypatch.setenv(
        "STEP_CA_PROVISIONER_PASSWORD_FILE", str(tmp_path / "wrong-name")
    )
...
def test_temp_dir_allowed_when_basename_matches(
    ...
    _enabled(monkeypatch)
    path = tmp_path / "pki-agent-news-creator-jwk"
...
def test_file_env_symlink_rejected(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    real = tmp_path / "real"
...
def test_pbes2_rejects_zip_and_unknown_header() -> None:
    ...
    with pytest.raises(PkiError, match="zip"):
        decrypt_provisioner_jwk(
            _compact_jwe_with_p2c(STEP_CA_PBES2_P2C, {"zip": "DEF"}), b"pw"
        )
    with pytest.raises(PkiError, match="header"):
        decrypt_provisioner_jwk(
            _compact_jwe_with_p2c(STEP_CA_PBES2_P2C, {"foo": "x"}), b"pw"
        )
    assert default_max_pbkdf2_iterations == before
...
def test_rejects_redirects(tmp_path: Path) -> None:
    ca = FakeStepCA("pki-agent-news-creator", "redirect-pw")
...
def test_http_client_requires_https(tmp_path: Path) -> None:
    iss = NativeStepCAIssuer(
        ...
        password_file=str(tmp_path / "pki-agent-news-creator-jwk"),
    )
    with pytest.raises(InsecureCAURLError):
        iss.issue(Ctx(), SUBJECT, [SUBJECT])
...
def test_rejects_tls12_only_ca(tmp_path: Path) -> None:
    ca = FakeStepCA("pki-agent-news-creator", "tls12-pw")
...
def test_ca_error_is_sentinel_without_body(tmp_path: Path) -> None:
    ...
    ca = FakeStepCA("pki-agent-news-creator", "sentinel-pw")
    ca.sign_status = 401
...
def test_provisioner_page_cap(tmp_path: Path) -> None:
    ca = FakeStepCA("pki-agent-news-creator", "pages-pw")
...
def test_response_size_cap(tmp_path: Path) -> None:
    ca = FakeStepCA("pki-agent-news-creator", "huge-pw")
    ca.sign_status = 201
...
def test_password_symlink_rejected(tmp_path: Path) -> None:
    ca = FakeStepCA("pki-agent-news-creator", "symlink-pw")
    url, root = ca.start(tmp_path)
    real = write_password_file(tmp_path, "pki-agent-news-creator-jwk", "symlink-pw")
...
def test_password_size_cap(tmp_path: Path) -> None:
    ca = FakeStepCA("pki-agent-news-creator", "cap-pw")
    url, root = ca.start(tmp_path)
    path = write_password_file(
        tmp_path, "pki-agent-news-creator-jwk", "x" * (MAX_PASSWORD_BYTES + 1)
    )
    iss = NativeStepCAIssuer(
        ca_url=url,
        root_file=root,
...
def test_rejects_extra_dns(tmp_path: Path) -> None:
    ca = FakeStepCA("pki-agent-news-creator", "extra-dns")
...
def test_exact_ip_and_uri_sans(tmp_path: Path) -> None:
    ca = FakeStepCA("pki-agent-news-creator", "typed-san")
    iss = _new_issuer(ca, tmp_path)
    sans = ["127.0.0.1", "spiffe://alt.local/news-creator"]
    try:
        ...
        assert san.get_values_for_type(cx509.UniformResourceIdentifier) == [
            "spiffe://alt.local/news-creator"
        ]
        assert key_pem
        usages = list(
            leaf.extensions.get_extension_for_class(cx509.ExtendedKeyUsage).value
        )
...
def test_rejects_mismatched_leaf_key(tmp_path: Path) -> None:
    ca = FakeStepCA("pki-agent-news-creator", "key-mismatch")
    iss = _new_issuer(ca, tmp_path)
    try:
        cert_pem, _key_pem = iss.issue(Ctx(), SUBJECT, [SUBJECT])
        other = ec.generate_private_key(ec.SECP256R1())
        with pytest.raises(PkiError, match="public key"):
            iss._validate_and_encode(
                SUBJECT, [SUBJECT], {"crt": cert_pem.decode()}, other
            )
    finally:
        ca.close()
...
def test_certfile_rejects_symlink_dest_and_parent(tmp_path: Path) -> None:
    ...
    cert, key = self_signed_pem("news-creator", nb, nb + timedelta(hours=1))
...
def test_enroll_cancel_during_backoff_is_cancelled(tmp_path: Path) -> None:
    from news_creator.infra.pki.config import Config
...
def test_rekey_does_not_use_named_tmp(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ca = FakeStepCA("pki-agent-news-creator", "memfd-pw")
    iss = _new_issuer(ca, tmp_path)
...
def test_prom_observer_rejects_default_registry() -> None:
    with pytest.raises(TypeError, match="private"):
        PromObserver("news-creator", DEFAULT_REGISTRY)
...
def test_start_uses_private_registry_and_ops(
    ...
    from news_creator.infra.pki.config import Config
    from news_creator.infra.pki.start import start_with_observer
    ...
    try:
        with urllib.request.urlopen(
            f"http://{handle.ops_addr}/health", timeout=2
        ) as health:
            assert health.status == 200
            assert b"news-creator" in health.read()
        with urllib.request.urlopen(
            f"http://{handle.ops_addr}/metrics", timeout=2
        ) as metrics:
            body = metrics.read().decode()
        assert "pki_enrollment_healthy" in body
        leaked = generate_latest(DEFAULT_REGISTRY).decode()
        assert (
            'subject="news-creator"' not in leaked
            or "pki_enrollment_healthy" not in leaked
        )
    finally:
        handle.stop()
...
def test_stop_during_http_closes_connections(tmp_path: Path) -> None:
    ca = FakeStepCA("pki-agent-news-creator", "http-stop")
...
def test_stop_during_write_does_not_abandon(tmp_path: Path) -> None:
    ...
    cfg = Config(
        mode=MODE_ENABLED,
        subject="news-creator-write",
        sans=("news-creator-write",),
        cert_path=str(files.cert_path),
        key_path=str(files.key_path),
        ca_url="https://127.0.0.1:1",
        root_file=str(tmp_path / "root.pem"),
        provisioner="pki-agent-news-creator-write",
        password_file="/run/secrets/pki-agent-news-creator-write-jwk",
        retry_attempts=1,
        retry_backoff=0.001,
        tick_interval=3600,
    )
...
def _leaf_and_ca(
    ...
    ca_builder = (
        ...
        .add_extension(
            cx509.SubjectKeyIdentifier.from_public_key(ca_pub), critical=False
        )
    )
    ca_cert = (
        ca_builder.sign(issuer_key, alg)
        if alg is not None
        else ca_builder.sign(issuer_key, None)
    )
    leaf_builder = (
        cx509.CertificateBuilder()
        ...
        .add_extension(
            cx509.SubjectAlternativeName([cx509.DNSName(cn)]), critical=False
        )
        .add_extension(
        ...
        .add_extension(
            cx509.SubjectKeyIdentifier.from_public_key(leaf_key.public_key()),
            critical=False,
        )
        .add_extension(
...
def test_ed25519_directly_signed_chain_cannot_bypass_policy() -> None:
    ...
    with pytest.raises(
        UnsupportedIssuerAlgorithm, match="unsupported issuer algorithm"
    ):
...
def test_non_ca_issuer_is_rejected() -> None:
    ...
    issuer = (
        ...
        .add_extension(
            cx509.SubjectKeyIdentifier.from_public_key(issuer_key.public_key()),
            critical=False,
        )
    ...
    leaf = (
        ...
        .not_valid_after(now + timedelta(hours=1))
        .add_extension(
            cx509.SubjectAlternativeName([cx509.DNSName("leaf.example")]),
            critical=False,
        )
        .add_extension(
            cx509.ExtendedKeyUsage(
...
def test_path_length_zero_rejects_intermediate() -> None:
    ...
    leaf_key = ec.generate_private_key(ec.SECP256R1())
    root_name = cx509.Name(
        [cx509.NameAttribute(NameOID.COMMON_NAME, "root-pl0.example")]
    )
    int_name = cx509.Name([cx509.NameAttribute(NameOID.COMMON_NAME, "int.example")])
    leaf_name = cx509.Name(
        [cx509.NameAttribute(NameOID.COMMON_NAME, "leaf-pl.example")]
    )
    root = (
        cx509.CertificateBuilder()
        ...
        .add_extension(
            cx509.SubjectKeyIdentifier.from_public_key(root_key.public_key()),
            critical=False,
        )
    ...
    intermediate = (
        ...
        .add_extension(
            cx509.SubjectKeyIdentifier.from_public_key(int_key.public_key()),
            critical=False,
        )
        .sign(root_key, hashes.SHA256())
    )
    leaf = (
        ...
        .not_valid_after(now + timedelta(hours=1))
        .add_extension(
            cx509.SubjectAlternativeName([cx509.DNSName("leaf-pl.example")]),
            critical=False,
        )
        .add_extension(
            cx509.ExtendedKeyUsage(
...
def test_name_constraints_reject_leaf() -> None:
    ...
    leaf_key = ec.generate_private_key(ec.SECP256R1())
    root_name = cx509.Name(
        [cx509.NameAttribute(NameOID.COMMON_NAME, "constrained-ca.example")]
    )
    ...
    leaf = (
        ...
        .not_valid_after(now + timedelta(hours=1))
        .add_extension(
            cx509.SubjectAlternativeName([cx509.DNSName("evil.example")]),
            critical=False,
        )
        .add_extension(
            cx509.ExtendedKeyUsage(
