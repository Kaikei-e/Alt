...
def test_prom_observer_exports_lifecycle_metrics() -> None:
    ...
    obs = PromObserver("news-creator", reg)
    obs.on_classified(State.FRESH, 8 * 3600)
    body = generate_latest(reg).decode()
    assert 'pki_enrollment_healthy{subject="news-creator"} 1.0' in body
    assert (
        'pki_enrollment_cert_remaining_seconds{subject="news-creator"} 28800.0' in body
    )
    ...
    body = generate_latest(reg).decode()
    for needle in (
        'pki_enrollment_healthy{subject="news-creator"} 0.0',
        'pki_enrollment_renewal_total{result="success",subject="news-creator"} 1.0',
        'pki_enrollment_renewal_total{result="failure",subject="news-creator"} 1.0',
        'pki_enrollment_reissue_total{reason="expired",subject="news-creator"} 1.0',
    ):
...
def test_prom_observer_rejects_default_registry() -> None:
    with pytest.raises(TypeError, match="private"):
        PromObserver("news-creator-metrics-nil", DEFAULT_REGISTRY)
...
def test_start_with_observer_wires_prom_when_enabled(tmp_path: Path) -> None:
    ...
    cfg = Config(
        mode=MODE_ENABLED,
        subject="news-creator-metrics",
        sans=("news-creator-metrics",),
        ...
        provisioner="pki-agent-news-creator-metrics",
        password_file="/run/secrets/pki-agent-news-creator-metrics-jwk",
    ...
    try:
        body = generate_latest(reg).decode()
        assert 'pki_enrollment_healthy{subject="news-creator-metrics"}' in body
