...
class NativeStepCAIssuer:
    ...
    def issue(self, ctx: Ctx, subject: str, sans: list[str]) -> tuple[bytes, bytes]:
        ...
        try:
            resp = self._post_json(
                ctx, "/sign", {"csr": csr_pem, "ott": ott}, client_cert=None
            )
    ...
    def rekey(
        ...
        try:
            resp = self._post_json(
                ctx, "/rekey", {"csr": csr_pem}, client_cert=(cert_pem, key_pem)
            )
        except CancelledError as exc:
    ...
    def _load_provisioner_jwk(
        self, ctx: Ctx, ssl_ctx: ssl.SSLContext, password: bytearray
    ) -> JWK:
        cursor = ""
    ...
    def _ssl_context(
        self, client_cert: tuple[bytes, bytes] | None = None
    ) -> ssl.SSLContext:
        try:
        ...
        try:
            ...
            raise PkiError(
                f"pki: parse CA root {self.root_file!r}: no certificates"
            ) from err
    ...
    def _validate_and_encode(
        ...
        if cn_value != subject:
            raise PkiError(
                f"pki: issued CN {cn_value!r} does not match subject {subject!r}"
            )
    ...
    def request(
        ...
        while time.monotonic() < deadline:
            ...
            conn = http.client.HTTPSConnection(
                host, port, context=ssl_ctx, timeout=slice_timeout
            )
            with self._lock:
...
def split_sans(
    ...
    list[str], list[ipaddress.IPv4Address | ipaddress.IPv6Address], list[str], list[str]
]:
    dns: list[str] = []
...
def encrypt_step_ca_provisioner_jwk(key: JWK, password: str) -> str:
    ...
    def _encrypt() -> str:
        protected = json.dumps(
            {"alg": _PBES2_ALG, "enc": _PBES2_ENC, "p2c": STEP_CA_PBES2_P2C}
        )
...
def _assert_sans(cert: x509.Certificate, sans: Sequence[str]) -> None:
    ...
    if sorted(got_dns) != sorted(item.lower() for item in want_dns):
        raise PkiError(
            f"pki: issued DNS SAN set mismatch: got {got_dns!r} want {list(want_dns)!r}"
        )
...
def _public_key_matches(
    leaf: x509.Certificate, key: ec.EllipticCurvePrivateKey
) -> None:
    leaf_spki = leaf.public_key().public_bytes(
        Encoding.DER, PublicFormat.SubjectPublicKeyInfo
    )
    csr_spki = key.public_key().public_bytes(
        Encoding.DER, PublicFormat.SubjectPublicKeyInfo
    )
...
def _verify_chain(
    ...
    try:
        ...
        raise PkiError(
            "pki: issued chain does not verify against pinned CA root"
        ) from err
    if (
    ...
    try:
        if dns:
            builder.build_server_verifier(x509.DNSName(dns[0])).verify(
                leaf, list(intermediates)
            )
        elif ips:
            builder.build_server_verifier(x509.IPAddress(ips[0])).verify(
                leaf, list(intermediates)
            )
        else:
        ...
        if "Forbidden public key algorithm" in msg or "Ed25519" in msg:
            raise UnsupportedIssuerAlgorithm(
                "pki: unsupported issuer algorithm"
            ) from err
        raise PkiError(
            "pki: issued chain does not verify against pinned CA root"
        ) from err
    ...
    try:
        ...
        if "exceeds" in str(err):
            raise PasswordTooLargeError(
                "pki: provisioner password file exceeded size cap"
            ) from err
        raise PkiError("pki: provisioner password file") from err
