"""Typed enrollment config. PKI_ENROLLMENT defaults to disabled when absent."""
...
class UnsupportedIssuerAlgorithm(PkiError):
    """Issuer public key algorithm is not accepted by WebPKI PolicyBuilder."""


MODE_ENABLED = "enabled"
MODE_DISABLED = "disabled"

DEFAULT_RENEW_AT = 0.66
...
class Config:
    ...
    provisioner: str
    password_file: str
    renew_at_fraction: float = DEFAULT_RENEW_AT
    tick_interval: float = DEFAULT_TICK_SECONDS
...
def load_config(service_name: str, environ: Mapping[str, str] | None = None) -> Config:
    ...
    renew_at = DEFAULT_RENEW_AT
    renew_raw = _get_env(env, "RENEW_AT_FRACTION", "")
    if renew_raw:
        try:
            renew_at = float(renew_raw)
        except ValueError as err:
            raise PKIConfigError(f"pki: RENEW_AT_FRACTION: {err}") from err
    ...
    cfg = Config(
        ...
        password_file=_get_env(
            env, "STEP_CA_PROVISIONER_PASSWORD_FILE", provisioner_password_file(subject)
        ),
        renew_at_fraction=renew_at,
        tick_interval=tick,
        retry_backoff=DEFAULT_BACKOFF_SECONDS,
        retry_attempts=DEFAULT_RETRIES,
    )
    _validate(cfg)
    if cfg.mode == MODE_ENABLED:
        _require_inbound_mtls(env)
    return cfg


def _require_inbound_mtls(env: Mapping[str, str]) -> None:
    if not _has(env, "INBOUND_MTLS"):
        raise PKIConfigError("pki: PKI_ENROLLMENT=enabled requires INBOUND_MTLS=true")
    raw = env["INBOUND_MTLS"].strip().lower()
    if raw not in {"true", "1", "on"}:
        raise PKIConfigError(
            "pki: PKI_ENROLLMENT=enabled requires INBOUND_MTLS=true "
            f"(got {env['INBOUND_MTLS']!r})"
        )
...
def _load_enrollment_mode(env: Mapping[str, str]) -> str:
    if _has(env, "PKI_ENROLLMENT_FILE"):
        ...
        if raw not in {MODE_ENABLED, MODE_DISABLED}:
            raise PKIConfigError(
                f"pki: PKI_ENROLLMENT={raw!r} must be {MODE_ENABLED!r} or {MODE_DISABLED!r}"
            )
        return raw
    ...
    if mode not in {MODE_ENABLED, MODE_DISABLED}:
        raise PKIConfigError(
            f"pki: PKI_ENROLLMENT={env['PKI_ENROLLMENT']!r} must be {MODE_ENABLED!r} or {MODE_DISABLED!r}"
        )
    return mode
...
def _parse_duration_seconds(raw: str) -> float:
    ...
    for suffix, multiplier in sorted(
        _DURATION_UNITS.items(), key=lambda item: -len(item[0])
    ):
        if text.endswith(suffix):
            ...
            try:
                ...
                raise PKIConfigError(
                    f"pki: PKI_ENROLLMENT_TICK_INTERVAL: {err}"
                ) from err
    try:
        return float(text)
    except ValueError as err:
        raise PKIConfigError(f"pki: PKI_ENROLLMENT_TICK_INTERVAL: {err}") from err
...
def _validate(cfg: Config) -> None:
    if not cfg.subject:
        raise PKIConfigError("pki: CERT_SUBJECT is required")
    if not 0.0 < cfg.renew_at_fraction < 1.0:
        raise PKIConfigError(
            f"pki: RENEW_AT_FRACTION must be in (0,1), got {cfg.renew_at_fraction}"
        )
    if cfg.mode != MODE_ENABLED:
        return
    ...
    if cfg.provisioner != want_prov:
        raise PkiError(
            f"pki: provisioner {cfg.provisioner!r} must be exactly {want_prov!r}"
        )
    ...
    if str(
        Path(cleaned).parent
    ) == "/run/secrets" and cleaned != provisioner_password_file(cfg.subject):
        raise PkiError(
    ...
    for name, value in required:
        if not value.strip():
            raise PKIConfigError(f"pki: {name} is required when PKI_ENROLLMENT=enabled")
