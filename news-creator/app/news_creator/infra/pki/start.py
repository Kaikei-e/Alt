...
from news_creator.infra.pki.config import MODE_ENABLED, Config, load_config
from news_creator.infra.pki.ctx import CancelledError, Ctx
from news_creator.infra.pki.manager import Issuer, Manager, NopObserver, Observer
from news_creator.infra.pki.metrics import PromObserver
from news_creator.infra.pki.ops import OpsHandle, start_ops
...
def start(service_name: str) -> Handle | None:
    ...
    return start_with_observer(
        cfg, None, PromObserver(cfg.subject, registry), registry=registry
    )
...
def start_with_observer(
    ...
    *,
    ...
    timeout = float(
        getattr(minted, "timeout", _DEFAULT_ISSUE_TIMEOUT) or _DEFAULT_ISSUE_TIMEOUT
    )
    return Handle(
        ctx,
        thread,
