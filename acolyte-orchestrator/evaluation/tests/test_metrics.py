...
class TestExtractShortIds:
    def test_returns_unique_short_ids_in_order(self) -> None:
...
class TestCitationPrecision:
    ...
    def test_no_citations_yields_none(self) -> None:
        body = "No citations."
        source_map: dict = {}
...
class TestLangMixRatio:
    ...
    def test_unknown_article_defaults_to_und(self) -> None:
...
class TestFaithfulness:
    ...
    def test_skips_when_no_citations(self) -> None:
        def fake_judge(prompt: str) -> float:
            raise AssertionError("judge should not be called")  # noqa: TRY003 — test guard, not a custom exception
