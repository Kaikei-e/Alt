...
a row forwarded but never marked, a wedged loop whose last gauge value still
reads healthy, a permanent failure retried forever.
"""
...
def _relay(outbox: _FakeOutbox, forwarder: _FakeForwarder, metrics: _FakeMetrics) -> RelayNotificationsUsecase:
...
async def test_forwards_each_claimed_row_then_marks_it() -> None:
    ...
    forwarded = await _relay(outbox, forwarder, metrics).tick()
...
async def test_claim_uses_the_configured_worker_id_and_batch_size() -> None:
    outbox = _FakeOutbox([[]])
    await _relay(outbox, _FakeForwarder(), _FakeMetrics()).tick()
...
async def test_one_bad_row_does_not_strand_the_rest_of_the_batch() -> None:
    ...
    relay = RelayNotificationsUsecase(
        outbox,  # type: ignore[arg-type]
        forwarder,  # type: ignore[arg-type]
        _FakeMetrics(),  # type: ignore[arg-type]
        worker_id="acolyte-1",
        batch_size=5,
    )
...
@pytest.mark.asyncio
async def test_the_backlog_gauge_is_emitted_every_tick_including_zero() -> None:
    outbox = _FakeOutbox([[], []], oldest_age=0.0)
    metrics = _FakeMetrics()
    relay = _relay(outbox, _FakeForwarder(), metrics)

    await relay.tick()
    await relay.tick()

    # A gauge you stop setting keeps reporting its last value, which makes a
    # wedged relay look healthy — so an empty backlog is still an emission.
    assert metrics.ages == [0.0, 0.0]
    assert len(metrics.ticks) == 2
    assert metrics.ticks[0] > 0


@pytest.mark.asyncio
async def test_the_backlog_gauge_reports_the_age_after_the_batch_drained() -> None:
    outbox = _FakeOutbox([[_pending()]], oldest_age=42.5)
    metrics = _FakeMetrics()

    await _relay(outbox, _FakeForwarder(), metrics).tick()

    assert metrics.ages == [42.5]
    kinds = [name for name, _ in outbox.events]
    assert kinds.index("forwarded") < kinds.index("age")


@pytest.mark.asyncio
async def test_a_failing_tick_does_not_stamp_the_liveness_gauge() -> None:
    ...
    metrics = _FakeMetrics()
    relay = _relay(_BrokenOutbox([]), _FakeForwarder(), metrics)

    with pytest.raises(RuntimeError):
        await relay.tick()

    assert metrics.ticks == []
...
async def test_the_loop_survives_a_failing_tick_and_keeps_going() -> None:
    ...
    outbox = _FlakyOutbox()
    relay = _relay(outbox, _FakeForwarder(), _FakeMetrics())
