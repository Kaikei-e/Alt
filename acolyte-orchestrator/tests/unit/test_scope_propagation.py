...
class FakeReportRepo:
    ...
    async def create_report(self, title: str, report_type: str) -> object:
        from acolyte.domain.report import Report
        from datetime import UTC, datetime

        rid = uuid4()
        report = Report(
            report_id=rid, title=title, report_type=report_type,
            current_version=0, latest_successful_run_id=None, created_at=datetime.now(UTC),
        )
        ...
        for i, s in enumerate(sections):
            if s.section_key == section_key:
                sections[i] = ReportSection(
                    report_id=report_id, section_key=section_key,
                    current_version=expected_version + 1, display_order=s.display_order,
                )
                break
