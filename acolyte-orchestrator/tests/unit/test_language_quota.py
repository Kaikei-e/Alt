...
class TestRebalanceByLanguage:
    def test_promotes_highest_scored_en_when_pool_has_en_and_curated_missing(self) -> None:
        curated = [
            _item("a", "ja", 0.9),
            _item("b", "ja", 0.85),
            _item("c", "ja", 0.7),
        ]
        pool = [*curated, _item("x", "en", 0.8), _item("y", "en", 0.5)]
    ...
    def test_no_change_when_pool_has_no_target_language(self) -> None:
        curated = [_item("a", "ja", 0.9), _item("b", "ja", 0.7)]
        pool = list(curated)
        result = rebalance_by_language(curated, pool, {"en": 0.2})
        assert [item["id"] for item in result] == ["a", "b"]

    def test_no_change_when_quota_already_met(self) -> None:
        curated = [
            _item("a", "ja", 0.9),
            _item("b", "en", 0.8),
            _item("c", "ja", 0.7),
    ...
    def test_respects_curated_length(self) -> None:
    ...
    def test_mutable_default_not_shared_across_calls(self) -> None:
    ...
    def test_empty_curated_returns_empty(self) -> None:
        assert rebalance_by_language([], [_item("x", "en", 0.9)], {"en": 0.2}) == []

    def test_quota_zero_means_no_enforcement(self) -> None:
        curated = [_item("a", "ja", 0.9), _item("b", "ja", 0.8)]
        pool = [*list(curated), _item("x", "en", 0.6)]
        result = rebalance_by_language(curated, pool, {"en": 0.0})
        assert [item["id"] for item in result] == ["a", "b"]

    def test_items_without_language_treated_as_und(self) -> None:
