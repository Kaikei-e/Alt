...
from acolyte.infra.peer_identity import PEER_IDENTITY_HEADER, PeerIdentityMiddleware
...
def _app(*, allowed: list[str] | None = None, strict: bool = True) -> Starlette:
...
async def test_spoofed_peer_header_is_ignored_for_tls_client(tmp_path: Path) -> None:
    ...
    server_cert, server_key = _write_signed_leaf(tmp_path, "acolyte-orchestrator", ca_cert, ca_key)
    client_cert, client_key = _write_signed_leaf(tmp_path, "alt-backend", ca_cert, ca_key)
    ctx = build_inbound_ssl_context(str(server_cert), str(server_key), str(ca_cert))
    app = _app(allowed=["alt-backend"], strict=True)
    server = await serve_inbound_tls(app, ctx, host="127.0.0.1", port=0, allowed_peers=("alt-backend",))
    try:
        ...
        body = resp.json()
        assert body["peer"] == "alt-backend"
        assert body["header"] == "alt-backend"
    finally:
        server.close()
...
async def test_untrusted_client_cert_never_reaches_app(tmp_path: Path) -> None:
    ...
    server_cert, server_key = _write_signed_leaf(tmp_path, "acolyte-orchestrator", ca_cert, ca_key)
    evil_cert, evil_key = _write_signed_leaf(tmp_path, "alt-backend", other_ca, other_key)
    ctx = build_inbound_ssl_context(str(server_cert), str(server_key), str(ca_cert))
    hits: list[str] = []
...
async def test_asgi_scope_tls_cn_survives_json_roundtrip(tmp_path: Path) -> None:
    """Sanity: the injected extension is what handlers see, not a wire header."""
    ca_cert, ca_key = _write_ca(tmp_path, "alt")
    server_cert, server_key = _write_signed_leaf(tmp_path, "acolyte-orchestrator", ca_cert, ca_key)
    client_cert, client_key = _write_signed_leaf(tmp_path, "bff", ca_cert, ca_key)
    ctx = build_inbound_ssl_context(str(server_cert), str(server_key), str(ca_cert))
    ...
    try:
        ...
        assert resp.json() == {"client_cn": "bff"}
    finally:
        server.close()
        await server.wait_closed()
