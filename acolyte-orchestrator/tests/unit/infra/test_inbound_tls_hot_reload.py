...
from acolyte.infra.inbound_tls import (
    InboundCertReloader,
...
def test_inbound_handshake_shows_new_not_after_after_atomic_replace(tmp_path: Path) -> None:
    ...
    cert, key, ca = _write_leaf(tmp_path, "acolyte-orchestrator", days=1)
    ...
    try:
        ...
        new_cert, new_key, _new_ca = _write_leaf(tmp_path, "acolyte-orchestrator", days=30)
        ...
        assert server._thread.is_alive(), "listener thread must survive cert rotation"
    finally:
        server.close()
...
def test_inbound_ssl_context_fails_closed_on_missing_cert(tmp_path: Path) -> None:
    _cert, key, ca = _write_leaf(tmp_path, "acolyte-orchestrator", days=1)
...
def test_inbound_ssl_context_fails_closed_on_missing_key(tmp_path: Path) -> None:
    cert, _key, ca = _write_leaf(tmp_path, "acolyte-orchestrator", days=1)
...
def test_inbound_ssl_context_fails_closed_on_empty_ca(tmp_path: Path) -> None:
    cert, key, _ca = _write_leaf(tmp_path, "acolyte-orchestrator", days=1)
...
def test_inbound_listener_rejects_untrusted_client_cert(tmp_path: Path) -> None:
    server_cert, server_key, server_ca = _write_leaf(tmp_path, "acolyte-orchestrator", days=1)
    ...
    try:
        ...
        with socket.create_connection(("127.0.0.1", server.port), timeout=5) as sock:
            # TLS 1.3 may complete wrap_socket on the client before the
            # server finishes CertificateVerify; the next I/O fails.
...
def test_inbound_reloader_keeps_previous_chain_on_garbage(tmp_path: Path) -> None:
    ...
    assert reloader.maybe_reload() is False
    # previous chain still serves
    client_cert = tmp_path / "client-cert.pem"
    client_key = tmp_path / "client-key.pem"
    # restore a valid client identity from the CA copy
    client_cert.write_bytes(ca.read_bytes())
    client_key.write_bytes(key.read_bytes())
    # key is still valid; cert file is garbage but reloader kept old chain.
    # Client still presents the original CA copy + original key — those still match.
    server = _TLSServer(ctx)
    server.start()
    try:
...
def test_resolve_inbound_tls_bind_disabled_when_unset() -> None:
    ...
    assert bind.plaintext_port == 8090
...
async def test_start_inbound_tls_listener_disabled_returns_none() -> None:
    bind = resolve_inbound_tls_bind(plaintext_host="127.0.0.1", plaintext_port=8090, environ={})
    app = Starlette(routes=[Route("/health", lambda _r: PlainTextResponse("ok"))])
