...
from acolyte.infra.pki.filesafe import MAX_PASSWORD_BYTES
from acolyte.infra.pki.manager import Manager
from acolyte.infra.pki.metrics import PromObserver
from acolyte.infra.pki.native_issuer import (
    STEP_CA_PBES2_P2C,
    NativeStepCAIssuer,
    decrypt_provisioner_jwk,
    encrypt_step_ca_provisioner_jwk,
)
from acolyte.infra.pki.start import start_enrollment
...
def _enabled(monkeypatch: pytest.MonkeyPatch, subject: str = "acolyte-orchestrator") -> None:
...
def test_empty_enrollment_fails_not_disabled(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(PkiError, match="PKI_ENROLLMENT"):
        load_config("acolyte-orchestrator")
...
def test_exact_provisioner_and_password_basename(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("STEP_CA_PROVISIONER", "pki-agent-acolyte-orchestrator")
    monkeypatch.setenv("STEP_CA_PROVISIONER_PASSWORD_FILE", "/run/secrets/pki-agent-acolyte-orchestrator-jwk")
...
def test_temp_dir_allowed_when_basename_matches(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    _enabled(monkeypatch)
    path = tmp_path / "pki-agent-acolyte-orchestrator-jwk"
    monkeypatch.setenv("STEP_CA_PROVISIONER_PASSWORD_FILE", str(path))
    cfg = load_config("acolyte-orchestrator")
...
async def test_rejects_redirects(tmp_path: Path) -> None:
    ...
    ca = FakeStepCA("pki-agent-acolyte-orchestrator", password)
    ca.redirect_sign = 307
    url, root = ca.start(tmp_path)
    pw = write_password_file(tmp_path, "pki-agent-acolyte-orchestrator-jwk", password)
...
async def test_http_client_requires_https(tmp_path: Path) -> None:
    iss = NativeStepCAIssuer(
        ...
        password_file=str(tmp_path / "pki-agent-acolyte-orchestrator-jwk"),
    )
    with pytest.raises(InsecureCAURLError):
        await iss.issue(SUBJECT, [SUBJECT])
...
async def test_password_symlink_rejected(tmp_path: Path) -> None:
    ...
    ca = FakeStepCA("pki-agent-acolyte-orchestrator", password)
    url, root = ca.start(tmp_path)
    real = write_password_file(tmp_path, "pki-agent-acolyte-orchestrator-jwk", password)
...
async def test_password_size_cap(tmp_path: Path) -> None:
    ...
    ca = FakeStepCA("pki-agent-acolyte-orchestrator", password)
    url, root = ca.start(tmp_path)
    path = write_password_file(tmp_path, "pki-agent-acolyte-orchestrator-jwk", "x" * (MAX_PASSWORD_BYTES + 1))
    iss = NativeStepCAIssuer(
...
async def test_exact_ip_and_uri_sans(tmp_path: Path) -> None:
    ...
    sans = ["127.0.0.1", "spiffe://alt.local/acolyte-orchestrator"]
    try:
        ...
        assert san.get_values_for_type(cx509.UniformResourceIdentifier) == ["spiffe://alt.local/acolyte-orchestrator"]
        assert key_pem
    finally:
        ca.close()
...
def test_certfile_rejects_symlink_dest_and_parent(tmp_path: Path) -> None:
    ...
    cert, key = self_signed_pem("acolyte-orchestrator", nb, nb + timedelta(hours=1))
...
def test_prom_observer_rejects_default_registry() -> None:
    with pytest.raises(TypeError, match="private"):
        PromObserver("acolyte-orchestrator", DEFAULT_REGISTRY)
...
async def test_start_uses_private_registry_and_ops(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("STEP_CA_PROVISIONER_PASSWORD_FILE", str(tmp_path / "pki-agent-acolyte-orchestrator-jwk"))
    ...
    handle = await start_enrollment("acolyte-orchestrator", issuer=_Issuer())
    ...
    try:
        ...
        assert "acolyte-orchestrator" in health.text
        assert metrics.status_code == 200
        assert "pki_enrollment_healthy" in metrics.text
        leaked = generate_latest(DEFAULT_REGISTRY).decode()
        assert 'subject="acolyte-orchestrator"' not in leaked or "pki_enrollment_healthy" not in leaked
    finally:
        await handle.aclose()
