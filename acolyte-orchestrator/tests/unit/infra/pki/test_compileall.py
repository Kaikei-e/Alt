...
from acolyte.infra.pki.config import MODE_DISABLED
...
PKI_DIR = ROOT / "acolyte" / "infra" / "pki"
INBOUND = ROOT / "acolyte" / "infra" / "inbound_tls.py"
MTLS = ROOT / "acolyte" / "infra" / "mtls_client.py"
...
async def test_disabled_mode_imports_and_starts(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    inbound = importlib.import_module("acolyte.infra.inbound_tls")
    mtls = importlib.import_module("acolyte.infra.mtls_client")
    manager = importlib.import_module("acolyte.infra.pki.manager")
    issuer = importlib.import_module("acolyte.infra.pki.native_issuer")
    ...
    log = _Log()
    handle = await start_enrollment("acolyte-orchestrator", logger=log)
    try:
...
def test_pki_direct_deps_are_capped() -> None:
    ...
    assert "httpx>=0.28.0,<0.29" in text
