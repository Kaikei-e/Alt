...
from acolyte.infra.pki.config import MODE_DISABLED, MODE_ENABLED, EnrollmentConfig, SharedRootSecretError
from acolyte.infra.pki.start import start_enrollment, start_enrollment_with
...
async def test_start_enabled_requires_inbound_tls_true(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(RuntimeError, match="INBOUND_TLS_ENABLED"):
        await start_enrollment("acolyte-orchestrator")
