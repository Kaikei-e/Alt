...
# Used in place of _CITATION_RULE when a claim carries no evidence_ids.
# Telling the model to cite [Sn] for an unsupported claim invites fabricated
# markers (see domain.citation_format.validate_citation_grounding).
_NO_CITATION_RULE = "- この分析ポイントには紐づく出典がない。[Sn] などの出典マーカーを本文中に一切使わないこと"
...
- 各分析ポイントの supporting_quotes を根拠として使うこと
...
def _build_contract_instructions(section: dict) -> str:
    ...
    if section.get("synthesis_only"):
        lines.append("- 既存の知見の統合のみ。新事実追加禁止")

    return "\n".join(lines)


def _format_claims(claims: list[dict], *, header: str = "計画済み分析ポイント:") -> str:
...
class WriterNode:
    ...
    async def _generate_paragraph(  # noqa: PLR0913 — keyword-only params are independent optional overrides, not a cohesive group worth a dataclass
        ...
        *,
        ...
        # A claim with no evidence_ids has nothing to cite — instructing
        ...
        if body:
            ...
            if is_valid:
                is_valid, reason = validate_citation_grounding(body, eids)
            if not is_valid:
                logger.warning(
    ...
    async def __call__(self, state: ReportGenerationState) -> dict:  # noqa: PLR0912, PLR0915 — LangGraph node orchestration, splitting would obscure the single control-flow narrative
        ...
        for section in non_es + es_sections:
            ...
            if use_claims and claim_plans is not None:
                ...
                evidence_block = format_evidence(section_evidence, hydrated)
