...
Plaintext ``host``:``port`` (uvicorn ``--host/--port``, default ``0.0.0.0:8090``)
    In-container health / sidecar dual-run upstream. Compose publishes it
    on loopback only (``127.0.0.1:8090``). It is not an external mTLS
    fallback.
...
def resolve_inbound_tls_bind(
    *,
    ...
    return InboundTLSBind(
        enabled=enabled,
        tls_host=env.get("INBOUND_TLS_HOST", "0.0.0.0"),
