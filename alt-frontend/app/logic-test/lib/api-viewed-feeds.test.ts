...
describe('ReadFeeds API Client - TDD Implementation', () => {
  ...
  describe('getReadFeedsWithCursor', () => {
    ...
    it('should fetch read feeds with default parameters', async () => {
      ...
      expect(mockGet).toHaveBeenCalledWith('/v1/feeds/fetch/viewed/cursor?limit=20', 10);
    ...
    it('should fetch read feeds with cursor and custom limit', async () => {
      ...
      expect(mockGet).toHaveBeenCalledWith('/v1/feeds/fetch/viewed/cursor?limit=50&cursor=cursor123', 15);
      expect(result.data).toHaveLength(2);
      expect(result.next_cursor).toBe('cursor123');
    });

    it('should validate limit constraints (minimum)', async () => {
      await expect(feedsApi.getReadFeedsWithCursor(undefined, 0)).rejects.toThrow('Limit must be between 1 and 100');
    });

    it('should validate limit constraints (maximum)', async () => {
      await expect(feedsApi.getReadFeedsWithCursor(undefined, 101)).rejects.toThrow('Limit must be between 1 and 100');
    });
    ...
    it('should handle API error responses', async () => {
      ...
      await expect(feedsApi.getReadFeedsWithCursor()).rejects.toThrow('API Error');
    });
  ...
  describe('prefetchReadFeeds', () => {
    it('should prefetch read feeds with given cursors', async () => {
      ...
      expect(mockGet).toHaveBeenCalledWith('/v1/feeds/fetch/viewed/cursor?limit=20&cursor=cursor1', 15);
      expect(mockGet).toHaveBeenCalledWith('/v1/feeds/fetch/viewed/cursor?limit=20&cursor=cursor2', 15);
      expect(mockGet).toHaveBeenCalledWith('/v1/feeds/fetch/viewed/cursor?limit=20&cursor=cursor3', 15);
    });
    ...
    it('should handle prefetch errors gracefully', async () => {
      ...
      await expect(feedsApi.prefetchReadFeeds(cursors)).resolves.toBeUndefined();
    });
  });
});