...
describe("useSSEProgress Hook - REAL BROWSER BEHAVIOR", () => {
  ...
  describe("ACTUAL PROBLEMATIC SCENARIOS", () => {
    it("should reproduce the mid-cycle reset bug with REAL timers", async () => {
      const { result, unmount } = renderHook(() => useSSEProgress(1000)); // 1 second for faster test
    ...
    it("should show timer overlap issues", async () => {
      const { result, unmount } = renderHook(() => useSSEProgress(1000));
      ...
      try {
        ...
        expect(progressAfterReset).toBeGreaterThan(20);
        expect(progressAfterReset).toBeLessThan(40);
      } finally {
        unmount(); // Ensure cleanup
      }
    }, 5000);
  ...
  describe("SIMULATE ACTUAL STATS PAGE USAGE", () => {
    it("should work exactly like the stats page component", async () => {
      ...
      const { result, unmount } = renderHook(() => useSSEProgress(5000)); // 5 second cycle like real app
