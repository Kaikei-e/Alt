...
import { DesktopFeedCard } from "../../../src/components/desktop/timeline/DesktopFeedCard";
...
describe("DesktopFeedCard", () => {
  ...
  it("should render feed with glass effect styling", () => {
    ...
    const card = screen.getByTestId(
      `desktop-feed-card-${mockDesktopFeeds[0].id}`,
    );
  ...
  it("should display feed metadata correctly", () => {
    ...
    expect(
      screen.getByText(mockDesktopFeeds[0].metadata.source.name),
    ).toBeInTheDocument();
  ...
  it("should handle favorite toggle action", () => {
    ...
    expect(mockProps.onToggleFavorite).toHaveBeenCalledWith(
      mockDesktopFeeds[0].id,
    );
  });
  ...
  it("should handle bookmark toggle action", () => {
    ...
    expect(mockProps.onToggleBookmark).toHaveBeenCalledWith(
      mockDesktopFeeds[0].id,
    );
  });
  ...
  it("should handle view article action", () => {
    ...
    expect(mockProps.onViewArticle).toHaveBeenCalledWith(
      mockDesktopFeeds[0].id,
    );
  });
  ...
  it("should show reading progress when feed is read", () => {
    ...
    renderWithChakra(<DesktopFeedCard {...mockProps} feed={readFeed} />);
