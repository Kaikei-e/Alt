...
vi.mock('./DynamicVirtualFeedList', () => ({
  DynamicVirtualFeedList: ({ feeds, onMeasurementError }: { feeds: Feed[], onMeasurementError: (error: Error) => void }) => (
...
describe('VirtualFeedListImpl', () => {
  const createFeed = (id: string, title: string, description: string): Feed => ({
    id,
    title,
    description,
  ...
  const longFeeds = Array.from({ length: 150 }, (_, i) => 
    createFeed(`long-${i}`, `Long Feed ${i}`, 'A'.repeat(600)) // Long description > 500 chars
  );
  ...
  it('should use fixed sizing for small feed count', () => {
    ...
    expect(screen.queryByTestId('dynamic-virtual-feed-list')).not.toBeInTheDocument();
  });
  ...
  it('should use fixed sizing when dynamic sizing is disabled', () => {
    ...
    renderWithChakra(<VirtualFeedListImpl {...defaultProps} feeds={longFeeds} />);
  ...
  it('should use dynamic sizing for large feed count with variable content', () => {
    ...
    expect(screen.queryByTestId('virtual-feed-list-core')).not.toBeInTheDocument();
  });
  ...
  it('should not use dynamic sizing for feeds without variable content', () => {
    ...
    const uniformFeeds = Array.from({ length: 150 }, (_, i) => 
      createFeed(`uniform-${i}`, `Feed ${i}`, 'Short description') // All have short descriptions
    );

    renderWithChakra(<VirtualFeedListImpl {...defaultProps} feeds={uniformFeeds} />);
  ...
  it('should persist error state after dynamic sizing error', async () => {
    const { rerender } = renderWithChakra(<VirtualFeedListImpl {...defaultProps} feeds={longFeeds} />);
    ...
    rerender(
      <ChakraProvider value={defaultSystem}>
        <VirtualFeedListImpl {...defaultProps} feeds={[...longFeeds, createFeed('new', 'New Feed', 'A'.repeat(600))] } />
      </ChakraProvider>
    );
