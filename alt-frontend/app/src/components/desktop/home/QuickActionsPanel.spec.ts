...
test.describe('QuickActionsPanel Component - PROTECTED', () => {
  ...
  test('should render with glass effect and action buttons grid (PROTECTED)', async ({ page }) => {
    const quickActionsPanel = page.locator('[data-testid="quick-actions-panel"]');

    await expect(quickActionsPanel).toBeVisible();
    
    // Verify glassmorphism visual properties
    const styles = await quickActionsPanel.evaluate(el => getComputedStyle(el));
    expect(styles.backdropFilter).toContain('blur');
    expect(styles.border).toContain('1px');
    
    // Check action buttons grid
    const actionsGrid = quickActionsPanel.locator('[data-testid="actions-grid"]');
  ...
  test('should display stats footer when additionalStats provided (PROTECTED)', async ({ page }) => {
    const quickActionsPanel = page.locator('[data-testid="quick-actions-panel"]');
    
    // Check stats footer is present
    const statsFooter = quickActionsPanel.locator('[data-testid="stats-footer"]');
  ...
  test('should not display stats footer when additionalStats not provided (PROTECTED)', async ({ page }) => {
