...
test.describe("DesktopSidebar Filters Mode - TASK1", () => {
  ...
  test("should use theme-aware colors for filter elements", async ({
    page,
  }) => {
    const readStatusLabel = page.locator(
    ...
    const labelStyles = await readStatusLabel.evaluate((el) =>
      getComputedStyle(el),
    );
  ...
  test("should support theme switching", async ({ page }) => {
    ...
    const filtersContainer = page.locator(
