...
test.describe('DesktopSidebar Filters Mode - TASK1', () => {
  ...
  test('should apply glassmorphism styling to filters container', async ({ page }) => {
    const filtersContainer = page.locator('[data-testid="desktop-sidebar-filters"]');
    
    // Verify glass class is applied
    await expect(filtersContainer).toHaveClass(/glass/);
    
    // Verify CSS variables are used for styling
    ...
    // Should use CSS variable values
    expect(styles.backdropFilter).toContain('blur');
    expect(styles.borderRadius).toBe('1rem'); // var(--radius-lg)
  });
  ...
  test('should use theme-aware colors for filter elements', async ({ page }) => {
    ...
    const filterButton = page.locator('[data-testid="filter-clear-button"]');
    
    // Verify theme-aware text colors are applied
    await expect(readStatusLabel).toHaveCSS('color', 'var(--text-primary)');
  ...
  test('should maintain filter functionality after UI updates', async ({ page }) => {
    ...
    // Test clear filters
    const clearButton = page.locator('[data-testid="filter-clear-button"]');
