...
test.describe("Desktop Feeds Page - PROTECTED", () => {
  ...
  test("should handle theme switching between vaporwave and liquid-beige (PROTECTED)", async ({
    page,
  }) => {
  ...
  test("should handle responsive layout across different viewports (PROTECTED)", async ({
    page,
  }) => {
    // Desktop layout (lg+)
  ...
  test("should handle filter interactions correctly (PROTECTED)", async ({
    page,
  }) => {
    ...
    const unreadFilter = page.locator(
      '[data-testid="sidebar-filter-read-status-unread"]',
    );
    ...
    const firstCheckbox = page
      .locator('[data-testid="filter-source-checkbox"]')
      .first();
    ...
    const todayFilter = page.locator(
      '[data-testid="sidebar-filter-time-range-today"]',
    );
    ...
    const allReadStatus = page.locator(
      '[data-testid="sidebar-filter-read-status-all"]',
    );
    const allTimeRange = page.locator(
      '[data-testid="sidebar-filter-time-range-all"]',
    );
  ...
  test("should maintain visual consistency across themes (PROTECTED)", async ({
    page,
  }) => {
    // Check liquid-beige theme contrast (default)
    const filterLabel = page.locator(
      '[data-testid="filter-read-status-label"]',
    );
    const liquidBeigeColor = await filterLabel.evaluate(
    ...
    const vaporwaveColor = await filterLabel.evaluate(
      (el) => getComputedStyle(el).color,
    );
    ...
    const sidebar = page.locator('[data-testid="desktop-sidebar-filters"]');
    const backdropFilter = await sidebar.evaluate(
      (el) => getComputedStyle(el).backdropFilter,
    );
    expect(backdropFilter).toContain("blur");
  });

  test("should handle sidebar collapse functionality (PROTECTED)", async ({
    page,
  }) => {
