...
describe('Automated Security Scan - PROTECTED', () => {
  describe('Component Security Scan - PROTECTED', () => {
    test('should detect potential XSS vulnerabilities in components - PROTECTED', async () => {
      ...
      const actualVulnerabilities = vulnerabilities.filter(v => 
        !v.includes('setTimeout with string detected') // 偽陽性のパターン
      );
    ...
    test('should detect unsafe URL handling - PROTECTED', async () => {
      ...
      for (const filePath of pageFiles) {
        ...
        if (content.includes('router.push(') && content.includes('${')) {
          vulnerabilities.push(`${relativePath}: potential unsafe router.push with template literal`);
        }
      }
  ...
  describe('API Security Scan - PROTECTED', () => {
    ...
    test('should verify CSP compliance in all pages - PROTECTED', async () => {
      ...
      expect(vulnerabilities.length).toBeLessThan(20); // 合理的な上限
    });
  });
      ...
      for (const [packageName, requiredVersion] of Object.entries(securityPackageRequirements)) {
        if (allDependencies[packageName]) {
          ...
          if (!currentVersion.includes(requiredVersion.replace('^', ''))) {
            vulnerabilities.push(`${packageName}: requires ${requiredVersion}, found ${currentVersion}`);
          }
        }
      }
  ...
  describe('Code Quality Security Scan - PROTECTED', () => {
    test('should verify proper error handling - PROTECTED', async () => {
      ...
      for (const filePath of allFiles) {
          ...
          if (tryBlocks) {
            for (const block of tryBlocks) {
              if (block.includes('console.error') && block.includes('error.message')) {
                vulnerabilities.push(`${relativePath}: potential information disclosure in error logging`);
              }
            }
          }
        }
        ...
        if (content.includes('.catch(') && content.includes('console.log')) {
          vulnerabilities.push(`${relativePath}: potential information disclosure in Promise error handling`);
        }
      }
    ...
    test('should verify proper type safety - PROTECTED', async () => {
      ...
      for (const filePath of allFiles) {
        ...
        // 非nullアサーション演算子の使用
        if (content.includes('!') && content.includes('.') && !content.includes('// @ts-expect-error')) {
          ...
          if (envUsages) {
            for (const usage of envUsages) {
              if (!usage.includes('NODE_ENV') && !usage.includes('NEXT_PUBLIC_')) {
                vulnerabilities.push(`${relativePath}: potential server-side environment variable exposure: ${usage}`);
              }
            }
          }
        }
