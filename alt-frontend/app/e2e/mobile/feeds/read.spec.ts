...
test.describe('既読記事ページ - PROTECTED', () => {
    ...
    await page.unroute('**/v1/feeds/fetch/read/cursor**');

    // Mock API responses for consistent testing
    await page.route('**/v1/feeds/fetch/read/cursor**', async (route) => {
    ...
    await page.goto('/mobile/feeds/read');
    await page.waitForLoadState('networkidle');
  });

  test('既読記事のみ表示される (PROTECTED)', async ({ page }) => {
    // Wait for data to load
    await expect(page.locator('[data-testid="read-feeds-scroll-container"]')).toBeVisible();

    // Verify page title - use more specific selector to avoid FloatingMenu link
    await expect(page.locator('[data-testid="read-feeds-title"]')).toBeVisible();
    await expect(page.locator('[data-testid="read-feeds-title"]')).toHaveText('Read Feeds');
    ...
    await expect(page.getByText('This is a test description for read feed 1')).toBeVisible();
    await expect(page.getByText('This is a test description for read feed 2')).toBeVisible();
  });
  ...
  test('アクセシビリティが正しく実装されている (PROTECTED)', async ({ page }) => {
    ...
    await expect(readFeedCard).toHaveAttribute('aria-label', 'Read feed: Test Read Feed 1');
    ...
    const feedLink = page.getByRole('link', { name: 'Open read feed Test Read Feed 1 in external link' }).first();
  ...
  test('空の状態が正しく表示される (PROTECTED)', async ({ page }) => {
    ...
    await expect(page.getByText('Mark some feeds as read to see them here')).toBeVisible();

    // Verify glass effect on empty state
    const emptyStateContainer = page.locator('.glass').filter({ hasText: 'No read feeds yet' });
