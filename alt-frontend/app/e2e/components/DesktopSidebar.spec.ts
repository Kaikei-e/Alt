...
test.describe("DesktopSidebar Component", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="desktop-sidebar"]', {
      timeout: 5000,
    });
  });

  test.describe("Navigation Mode", () => {
    test("should render logo and navigation items correctly", async ({
      page,
    }) => {
    ...
    test("should highlight active navigation item", async ({ page }) => {
      ...
      const hasActiveClass =
        (await activeContainer.locator(".active").count()) > 0;
  ...
  test.describe("Feeds Filter Mode", () => {
    ...
    test("should display filters header and collapse toggle", async ({
      page,
    }) => {
    ...
    test("should display read status filter options", async ({ page }) => {
      ...
      await expect(
        page.getByTestId("sidebar-filter-read-status-all"),
      ).toBeVisible();
      await expect(
        page.getByTestId("sidebar-filter-read-status-unread"),
      ).toBeVisible();
      await expect(
        page.getByTestId("sidebar-filter-read-status-read"),
      ).toBeVisible();
    });
    ...
    test("should display time range filter options", async ({ page }) => {
      ...
      await expect(
        page.getByTestId("sidebar-filter-time-range-all"),
      ).toBeVisible();
      await expect(
        page.getByTestId("sidebar-filter-time-range-today"),
      ).toBeVisible();
      await expect(
        page.getByTestId("sidebar-filter-time-range-week"),
      ).toBeVisible();
      await expect(
        page.getByTestId("sidebar-filter-time-range-month"),
      ).toBeVisible();
    });
    ...
    test("should apply glassmorphism styling in filter mode", async ({
      page,
    }) => {
