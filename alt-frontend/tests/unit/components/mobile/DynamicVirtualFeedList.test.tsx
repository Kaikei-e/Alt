...
import { DynamicVirtualFeedList } from '../../../src/DynamicVirtualFeedList";
...
vi.mock("@/utils/sizeMeasurement", () => ({
  SizeMeasurementManager: vi.fn().mockImplementation(() => ({
    measureElement: vi
      .fn()
      .mockResolvedValue({ height: 200, width: 100, timestamp: Date.now() }),
    clearCache: vi.fn(),
    getEstimatedSize: vi.fn(
      (contentLength) => 120 + Math.ceil(contentLength / 50) * 24,
    ),
  })),
}));
...
describe("DynamicVirtualFeedList", () => {
  ...
  const longFeed: Feed = {
    ...
    description:
      "This is a very long description that contains multiple sentences and should result in a taller feed card. ".repeat(
        5,
      ),
  ...
  it("should render dynamic virtual scroll container", () => {
    ...
    expect(
      screen.getByTestId("dynamic-virtual-content-container"),
    ).toBeInTheDocument();
  });
  ...
  it("should maintain scroll position during dynamic sizing", async () => {
    const { rerender } = renderWithChakra(
      <DynamicVirtualFeedList {...defaultProps} feeds={[shortFeed]} />,
    );
  ...
  it("should handle empty feeds gracefully", () => {
    renderWithChakra(<DynamicVirtualFeedList {...defaultProps} feeds={[]} />);

    expect(screen.getByText("No feeds available")).toBeInTheDocument();
    expect(
      screen.getByTestId("dynamic-virtual-empty-state"),
    ).toBeInTheDocument();
  });

  it("should use overscan parameter correctly", () => {
    renderWithChakra(<DynamicVirtualFeedList {...defaultProps} overscan={3} />);
  ...
  it("should call onMarkAsRead when feed is marked as read", () => {
    ...
    renderWithChakra(
      <DynamicVirtualFeedList {...defaultProps} onMarkAsRead={onMarkAsRead} />,
    );
  ...
  it("should render with proper container height", () => {
    renderWithChakra(
      <DynamicVirtualFeedList {...defaultProps} containerHeight={800} />,
    );
