...
import { VirtualDesktopTimeline } from '../../../src/VirtualDesktopTimeline";
...
vi.mock("./DesktopFeedCard", () => ({
  DesktopFeedCard: ({
    ...
    <div data-testid={`desktop-feed-card-${feed.id}`}>
      ...
      <button onClick={() => onMarkAsRead(feed.id)} aria-label="Mark as read">
        Mark as read
      </button>
      <button
      ...
      <button onClick={() => onReadLater(feed.id)} aria-label="Read later">
        Read later
      </button>
      <button onClick={() => onViewArticle(feed.id)} aria-label="View article">
        View article
      </button>
    </div>
  ),
...
describe("VirtualDesktopTimeline", () => {
  ...
  it("should adapt to desktop viewport changes", () => {
    const { rerender } = renderWithChakra(
      <VirtualDesktopTimeline {...defaultProps} containerHeight={600} />,
    );

    const smallViewportContainer = screen.getByTestId(
      "virtual-desktop-timeline",
    );
    ...
    rerender(
      <ChakraProvider value={defaultSystem}>
        <VirtualDesktopTimeline {...defaultProps} containerHeight={1200} />
      </ChakraProvider>,
    );

    const largeViewportContainer = screen.getByTestId(
      "virtual-desktop-timeline",
    );
  ...
  it("should handle empty feeds gracefully", () => {
    renderWithChakra(<VirtualDesktopTimeline {...defaultProps} feeds={[]} />);

    expect(
      screen.getByTestId("virtual-desktop-empty-state"),
    ).toBeInTheDocument();
  ...
  it("should filter out read feeds", () => {
    ...
    renderWithChakra(
      <VirtualDesktopTimeline {...defaultProps} readFeeds={readFeeds} />,
    );

    // 既読フィードは表示されない
    expect(
      screen.queryByTestId(`desktop-feed-card-${mockDesktopFeeds[0].id}`),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByTestId(`desktop-feed-card-${mockDesktopFeeds[1].id}`),
    ).not.toBeInTheDocument();

    // 未読フィードは表示される
    expect(
      screen.getByTestId(`desktop-feed-card-${mockDesktopFeeds[2].id}`),
    ).toBeInTheDocument();
  });

  it("should handle dynamic sizing when enabled", () => {
    renderWithChakra(
      <VirtualDesktopTimeline {...defaultProps} enableDynamicSizing={true} />,
    );
  ...
  it("should use fixed sizing when dynamic sizing is disabled", () => {
    renderWithChakra(
      <VirtualDesktopTimeline {...defaultProps} enableDynamicSizing={false} />,
    );
  ...
  it("should handle overscan parameter correctly", () => {
    renderWithChakra(<VirtualDesktopTimeline {...defaultProps} overscan={3} />);
  ...
  it("should call onMarkAsRead when mark as read button is clicked", async () => {
    ...
    renderWithChakra(
      <VirtualDesktopTimeline {...defaultProps} onMarkAsRead={onMarkAsRead} />,
    );
  ...
  it("should handle large number of feeds efficiently", () => {
    ...
    renderWithChakra(
      <VirtualDesktopTimeline {...defaultProps} feeds={largeFeeds} />,
    );
