...
import { sanitizeContent, sanitizeFeedContent } from '../../../src/contentSanitizer";
  ...
  describe("sanitizeContent", () => {
    ...
    it("should remove dangerous script tags with attributes", () => {
      const maliciousContent =
        '<script src="malicious.js" type="text/javascript">alert("xss")</script>Safe content';
      expect(sanitizeContent(maliciousContent)).toBe("Safe content");
    });

    it("should preserve safe HTML tags", () => {
      const safeContent = "<b>Bold</b> and <i>italic</i> text";
      expect(sanitizeContent(safeContent)).toBe(
    ...
    it("should preserve strong and em tags", () => {
      ...
      expect(sanitizeContent(content)).toBe(
        "<strong>Strong</strong> and <em>emphasis</em> text",
      );
    });
    ...
    it("should remove dangerous HTML tags", () => {
      const dangerousContent =
        '<iframe src="malicious.html"></iframe>Text content';
      expect(sanitizeContent(dangerousContent)).toBe("Text content");
    });

    it("should remove style and form tags", () => {
      const dangerousContent =
        "<style>body{display:none;}</style><form>form content</form>Text";
