...
import {
  csrfAutoRecovery,
  withAutoRecovery,
} from "@/lib/auth/csrf-auto-recovery";
...
describe("CSRF Token Integration Tests", () => {
  ...
  describe("Phase 1: Basic CSRF Token Flow", () => {
    test(
      "should create login flow and extract CSRF token",
      async () => {
        // 1. Create login flow
    ...
    test(
      "should include CSRF token in login request body",
      async () => {
        ...
        const csrfToken =
          ...
          "mock-csrf-token-123456789012345678901234567890";
  ...
  describe("Phase 2: AuthAPI Integration", () => {
    test(
      "should use enhanced auth API with CSRF token extraction",
      async () => {
        ...
        try {
          ...
          console.log("📊 AuthAPI integration test result:", {
            ...
            isCSRFError: (error instanceof Error ? error.message : "")
              .toLowerCase()
              .includes("csrf"),
          });
  ...
  describe("Phase 3: Auto-Recovery System", () => {
    test(
      "should automatically retry on CSRF errors",
      async () => {
  ...
  describe("Phase 4: Debug Endpoint Integration", () => {
    ...
    test("should detect missing CSRF token through debug endpoint", async () => {
      ...
      expect(diagnostic.validation_result).toBe(
        "CSRF_TOKEN_MISSING_OR_INVALID",
      );
      expect(diagnostic.compliance_check.overall_compliant).toBe(false);
      expect(diagnostic.recommendations).toContain(
        "🚨 Include csrf_token field in request body",
      );
    });
  });
...
describe("CSRF Performance and Load Tests", () => {
  test("should handle multiple concurrent login attempts", async () => {
    ...
    console.log("🚀 Concurrent login performance:", {
      ...
      successfulAttempts: results.filter((r) => r.status === "fulfilled")
        .length,
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        ...
        if (!errorMessage.includes("redirect")) {
          console.warn(
            `Iteration ${i} failed with non-redirect error:`,
            errorMessage,
          );
        }
      }
