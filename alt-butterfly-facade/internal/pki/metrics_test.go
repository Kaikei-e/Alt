...
func TestPromObserver_ExportsLifecycleMetrics(t *testing.T) {
	...
	obs := NewPromObserver("alt-butterfly-facade", reg)

	obs.OnClassified(StateFresh, 8*time.Hour)
	body := gather(t, reg)
	if !strings.Contains(body, `pki_enrollment_healthy{subject="alt-butterfly-facade"} 1`) {
		t.Fatalf("healthy missing:\n%s", body)
	}
	if !strings.Contains(body, `pki_enrollment_cert_remaining_seconds{subject="alt-butterfly-facade"} 28800`) {
	...
	for _, needle := range []string{
		`pki_enrollment_healthy{subject="alt-butterfly-facade"} 0`,
		`pki_enrollment_renewal_total{result="success",subject="alt-butterfly-facade"} 1`,
		`pki_enrollment_renewal_total{result="failure",subject="alt-butterfly-facade"} 1`,
		`pki_enrollment_reissue_total{reason="expired",subject="alt-butterfly-facade"} 1`,
	} {
...
func TestPromObserver_NilRegistererPanics(t *testing.T) {
	...
	_ = NewPromObserver("alt-butterfly-facade-metrics-nil", nil)
}
...
func TestStartWithObserver_WiresPromObserverWhenEnabled(t *testing.T) {
	...
	cfg := &Config{
		Mode:            ModeEnabled,
		Subject:         "alt-butterfly-facade-metrics",
		SANs:            []string{"alt-butterfly-facade-metrics"},
		CertPath:        dir + "/svc-cert.pem",
		KeyPath:         dir + "/svc-key.pem",
		Provisioner:     "pki-agent-alt-butterfly-facade-metrics",
		PasswordFile:    "/run/secrets/pki-agent-alt-butterfly-facade-metrics-jwk",
	...
	defer h.Stop()
	body := gather(t, reg)
	if !strings.Contains(body, `pki_enrollment_healthy{subject="alt-butterfly-facade-metrics"} 1`) {
