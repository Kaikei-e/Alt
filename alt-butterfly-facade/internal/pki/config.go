// Package pki is the in-process certificate lifecycle for alt-butterfly-facade.
//
// It is intentionally independent of the pki-agent module and of
// alt-backend's internal/pki: no replace directive, no shared Go import.
// The state machine, atomic writes and subject-scoped config live here and
// are tested with a fake issuer, fake clock and a temp filesystem. Minting
// against step-ca uses NativeStepCAIssuer (official jose + provision token
// APIs, no step CLI) when PKI_ENROLLMENT=enabled.
package pki
...
var (
	...
	// ErrNotHTTPS is F-002: STEP_CA_URL must be https.
	ErrNotHTTPS = errors.New("pki: STEP_CA_URL must use https")
	...
	// ErrResponseTooLarge is F-003: a CA body exceeded maxResponseBytes.
	ErrResponseTooLarge = errors.New("pki: CA response exceeded size cap")
	// ErrProvisionerPageLimit is F-003: /provisioners pagination exceeded the page cap.
...
func (c *Config) validate() error {
	...
	if err != nil || u.Scheme != "https" || u.Host == "" {
		return fmt.Errorf("%w (got %q)", ErrNotHTTPS, c.CAURL)
	}
	return nil
}
