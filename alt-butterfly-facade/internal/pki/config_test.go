...
func TestLoadConfig_DefaultDisabled(t *testing.T) {
	...
	if c.Subject != "alt-butterfly-facade" {
...
func TestLoadConfig_EnabledRejectsSharedProvisioner(t *testing.T) {
	...
	t.Setenv("STEP_CA_PROVISIONER_PASSWORD_FILE", "/run/secrets/pki-agent-alt-butterfly-facade-jwk")
...
func TestLoadConfig_EnabledSubjectScopedDefaults(t *testing.T) {
	...
	if c.Provisioner != "pki-agent-alt-butterfly-facade" {
		t.Fatalf("provisioner=%q", c.Provisioner)
	}
	if got := c.PasswordFile; got != "/run/secrets/pki-agent-alt-butterfly-facade-jwk" {
...
func TestLoadConfig_DistinctSubjectsDoNotShareIdentity(t *testing.T) {
	t.Setenv("PKI_ENROLLMENT", ModeEnabled)
	t.Setenv("CERT_SUBJECT", "alt-butterfly-facade")
	a, err := LoadConfig("alt-butterfly-facade")
	if err != nil {
		t.Fatal(err)
	}
