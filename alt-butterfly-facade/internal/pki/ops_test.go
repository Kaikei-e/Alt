...
func TestNewOpsHandler_NilMetrics503(t *testing.T) {
	h := NewOpsHandler("alt-butterfly-facade", nil)
...
func TestNewOpsHandler_PrivateRegistryMetrics(t *testing.T) {
	...
	obs := NewPromObserver("alt-butterfly-facade-ops", reg)
	obs.OnClassified(StateFresh, time.Hour)
	h := NewOpsHandler("alt-butterfly-facade", promhttp.HandlerFor(reg, promhttp.HandlerOpts{}))
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/metrics", nil))
	if rec.Code != http.StatusOK {
		t.Fatalf("status=%d", rec.Code)
	}
	if !strings.Contains(rec.Body.String(), `pki_enrollment_healthy{subject="alt-butterfly-facade-ops"} 1`) {
...
func TestListenOps_ShutdownIdempotent(t *testing.T) {
	...
	srv, err := ListenOps(context.Background(), nil, "alt-butterfly-facade", nil)
	if err != nil {
		t.Fatal(err)
	}
