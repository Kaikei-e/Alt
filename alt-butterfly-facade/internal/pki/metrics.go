...
// metric family on a dedicated private registry scraped from :9110.
// Cardinality is one series per subject.
...
// NewPromObserver registers collectors on a dedicated private registry.
// A nil registerer panics: DefaultRegisterer would leak PKI series onto the
// application mux (F-001).
func NewPromObserver(subject string, reg prometheus.Registerer) *PromObserver {
	if reg == nil {
		panic("pki: NewPromObserver requires a dedicated prometheus.Registerer")
	}
