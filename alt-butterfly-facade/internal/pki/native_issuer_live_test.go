...
const (
	...
	liveCAContainer = "alt-pki-native-itest-bff"
	liveCANetwork   = "alt-pki-native-itest-bff-net"
	liveCAHostPort  = "19001"
	liveCAPassword  = "itest-only"
	liveJWKPassword = "itest-jwk-alt-butterfly-facade"
	liveProvisioner = "pki-agent-alt-butterfly-facade"
	liveSubject     = "alt-butterfly-facade"
)
...
//
//	docker network create alt-pki-native-itest-bff-net
//	docker run --rm -d --name alt-pki-native-itest-bff \
//	  --network alt-pki-native-itest-bff-net \
...
//	  -e DOCKER_STEPCA_INIT_WITH_CA_URL=https://127.0.0.1:19001 \
...
func TestNativeStepCAIssuer_DisposableLiveCA(t *testing.T) {
	...
	passwordFile := writePasswordFile(t, tmp, "pki-agent-alt-butterfly-facade-jwk", liveJWKPassword, 0o400)
...
printf '%%s\n' %q > /tmp/pki-agent-alt-butterfly-facade-jwk
chmod 400 /tmp/pki-agent-alt-butterfly-facade-jwk
step ca provisioner add %q --type JWK --create \
  --password-file /tmp/pki-agent-alt-butterfly-facade-jwk \
...
func mustLiveLeaf(t *testing.T, certPEM, keyPEM []byte, rootFile string) *x509.Certificate {
	...
	if err := certHasExactSANs(leaf, []string{liveSubject}); err != nil {
		t.Fatal(err)
	}
