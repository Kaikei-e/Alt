...
1. **型ヒント必須**: 公開関数・メソッドは完全アノテーション。`Any` は境界最小限。`mypy --strict` / pyright strict 通過必須
...
4. **Ruff + mypy が一次ソース**: フォーマット・静的検査はツールで自動化。推奨ルール集合 `E,W,F,B,UP,SIM,N,I,ANN,S,PTH,C4`。手動スタイル議論禁止
