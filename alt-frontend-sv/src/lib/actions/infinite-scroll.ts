...
): { update: (newOptions: InfiniteScrollOptions) => void; destroy: () => void } {
      ...
      observer = new IntersectionObserver(
        async (entries) => {
          ...
          try {
            ...
            if (observer && element && element.isConnected && !currentOptions.disabled) {
              try {
  ...
  return {
    update(newOptions: InfiniteScrollOptions) {
      ...
      if (
        ...
        setupObserver();
      } else if (!observer && !newOptions.disabled && element && element.isConnected && !isSettingUp) {
