...
describe("useKnowledgeLoopStream — lifecycle", () => {
	...
	it("does not schedule a phantom reconnect when the effect tears down (intentional abort)", {
		timeout: 6000,
	}, async () => {
	...
	it("closes the abort-aware stream when the effect cleanup runs", {
		timeout: 4000,
	}, async () => {
		let abortedFromHook = false;
		mockStreamFn.mockImplementation((_req, opts: { signal: AbortSignal }) => {
	...
	it("resumes from a saved cursor across an effect-driven remount", {
		timeout: 4000,
	}, async () => {
		...
		// remount can resume.
		sessionStorage.setItem("knowledge-loop:resume:user-A:default", "42");
		cleanupA();
