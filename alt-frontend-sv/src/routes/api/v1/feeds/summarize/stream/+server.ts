...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	if (!locals.session) {
		console.warn(
			"[StreamSummarize] Authentication failed: no session (early check)",
		);
		return json(
			{
	...
	try {
		...
		if (!token) {
			console.warn(
				"[StreamSummarize] Authentication failed: no backend token (early check)",
			);
			return json(
				{
					error: "Authentication required",
	...
	try {
		...
		if (
			(!body.feed_url || typeof body.feed_url !== "string") &&
			(!body.article_id || typeof body.article_id !== "string")
		) {
			console.warn(
				"[StreamSummarize] Validation failed: missing feed_url or article_id",
			);
			return json(
		...
		if (body.feed_url) {
			try {
				...
				console.log("[StreamSummarize] SSRF validation passed", {
					feed_url: body.feed_url,
				});
			} catch (error) {
				if (error instanceof Error && error.name === "SSRFValidationError") {
					console.warn("[StreamSummarize] SSRF validation failed", {
						feed_url: body.feed_url,
						error: error.message,
					});
					return json(
		...
		if (!backendResponse.ok) {
			...
			try {
				...
				console.warn("[StreamSummarize] Failed to read error response body", {
					error: e,
				});
			}
		...
		const stream = new ReadableStream({
			async start(controller) {
				...
				try {
					console.log(
						"[StreamSummarize] Stream reader started (direct streaming)",
						{
							hasBody: !!backendResponse.body,
							contentType: backendResponse.headers.get("Content-Type"),
						},
					...
					console.log("[StreamSummarize] Stream completed", {
						...
						avgChunkSize:
							chunksForwarded > 0
								? Math.round(bytesForwarded / chunksForwarded)
								: 0,
					});
					controller.close();
				} catch (error) {
					...
					try {
						...
						console.error("[StreamSummarize] Failed to encode error message", {
							encodeError,
						});
					}
					controller.close();
		...
		console.error(
			"[StreamSummarize] Unexpected error in endpoint",
			errorDetails,
		);
