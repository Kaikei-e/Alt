...
function makeItem(
	...
	return {
		...
		summaryExcerpt: "記事要約がここに入る。タイトル同様、body へ載せると WAF の反応面を広げる。",
		summaryState: "ready",
		tags: [],
		why: [],
		score: 0.5,
...
describe("buildHomeActionMetadata", () => {
	it.each(["dismiss", "open", "open_recap", "open_search", "ask", "listen"])(
		"returns undefined for %s so article content never reaches the POST body",
		(type) => {
	...
	it("does not leak summaryExcerpt even when populated", () => {
		const item = makeItem({ summaryExcerpt: "contains OWASP-CRS trigger words" });
		const result = buildHomeActionMetadata("dismiss", item);
		expect(result).toBeUndefined();
	});
});
