...
const server = http.createServer((req, res) => {
  ...
  if (path === "/sessions/whoami") {
    ...
    if (cookie && cookie.includes("ory_kratos_session=e2e-session")) {
      ...
      res.end(JSON.stringify({ error: { code: 401, message: "No active session found" } }));
    }
    return;
  }
...
server.on("error", (e) => {
  if (e.code === "EADDRINUSE") {
    console.log(`[kratos-mock] Port ${PORT} already in use, assuming server is running.`);
    process.exit(0);
  } else {
...
server.listen(PORT, "127.0.0.1", () => {
  console.log(`[kratos-mock] specialized Kratos mock listening on port ${PORT}`);
});
