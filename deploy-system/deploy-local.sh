...
# DOCKER_GROUP_ID was historically required by compose/logging.yaml before the
# socket-proxy change; retained here as an environment guard so host-level
# Docker group permissions remain validated before build/deploy. Fail fast here
# with remediation if unset.
if [ -z "${DOCKER_GROUP_ID:-}" ]; then
    log "ERROR: DOCKER_GROUP_ID is not set."
    log "       Required up front for the docker-group preflight (no compose file interpolates it any more)."
