---
title: "Desktop検索 Infinite Scroll を `bind:this` + `$effect` パターンに移行"
date: 2026-02-11
status: accepted
tags: [docker, frontend, performance]
affected_services: [alt-frontend-sv]
aliases: [ADR-222, ADR-000222]
---
...
[[000219|ADR-000219]]〜221 で Desktop `/sv/feeds/search` の infinite scroll スピナー永続表示バグに段階的に対処してきた。しかし、`use:infiniteScroll` Svelte action を使用したアーキテクチャの構造的問題が根本原因として残っていた。
...
4. [[000219|ADR-000219]]〜221 で段階的に追加されたクールダウン・disabled toggle 等の対症療法が不要になり、コードが簡潔になった
...
- 前提: [[000219|ADR-000219]]（クールダウン追加・検索クライアント改善）、[[000220|ADR-000220]]（レイテンシ改善）、[[000221|ADR-000221]]（observer 再作成コスト削減）
