---
title: RAG Orchestrator TTFT ボトルネック修正 (計測ベース最適化)
date: 2026-02-10
status: accepted
tags: [ai, performance, rag]
affected_services: [rag-orchestrator]
aliases: [ADR-209, ADR-000209]
---
...
[[000208|ADR-208]] で空レスポンス修正と TTFT 高速化（パイプライン並列化・タイムアウト最適化・接続プール導入）を実施した。空レスポンス問題は解消し、982 文字の回答生成に成功したが、実計測の結果 TTFT が 76〜93 秒と目標（10 秒）に大幅未達であることが判明した。
