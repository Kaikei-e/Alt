---
title: Feature Model導入によるスタック依存関係の改善
date: 2026-01-08
status: accepted
tags: [authentication, logging, rag, recap, search]
affected_services: [altctl, auth-hub, nginx, rask-log-forwarder]
...
[[000057|ADR 000057]]で検討されたように、`altctl up core`を実行した際に検索機能が動作しない問題が発生していた。これは`core`スタックと`workers`スタックが構造的には独立しているが、検索機能は両方を必要とするという「機能的依存関係」の問題だった。
...
- [[000057]] 検索機能のエラーハンドリング改善とスタック依存関係の明確化
