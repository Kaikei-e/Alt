---
title: OGP 画像即時表示システム — 実装完了まとめ
date: 2026-03-02
status: accepted
tags: [caching, connect-rpc, queue, rag, search]
affected_services: [alt-backend, alt-frontend-sv, auth-hub, pre-processor, rag-orchestrator, search-indexer]
aliases: [ADR-310, ADR-000310]
---
...
[[000307|ADR-307]]〜309 を通じて OGP 画像即時表示システムを段階的に構築した:

- **[[000307|ADR-307]]**: RSS XML からの画像 URL 抽出 + バックグラウンドプリフェッチ（`ogp-image-warmer` ジョブ）
- **[[000308|ADR-308]]**: 画像プロキシの動的ドメイン許可にサブドメインマッチングを導入
- **[[000309|ADR-309]]**: visual-preview モードで OGP 画像が表示されないバグの修正（`connectFeedToRenderFeed` のフィールド欠落）
...
### 変更対象ファイル（[[000307|ADR-307]]〜309 で未記載分）
...
- [[000030]] 7 : RSS XML からの OGP 画像 URL 抽出とバックグラウンドプリフェッチ
- [[000030]] 8 : 画像プロキシのドメイン許可リストにサブドメインマッチングを導入
- [[000030]] 9 : visual-preview OGP 画像非表示バグの修正
- [[000337]] ArticlePrefetcher キャッシュエビクションによる OG 画像消失バグの修正（第2報）
