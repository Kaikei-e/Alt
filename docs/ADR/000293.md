---
title: ハートビートをLLMストリーミングフェーズに拡張
date: 2026-02-28
status: accepted
tags: [ai, frontend, rag, testing]
affected_services: [nginx, rag-orchestrator]
aliases: [ADR-293, ADR-000293]
---
# ADR-000293: ハートビートをLLMストリーミングフェーズに拡張

## ADR's STATUS

Accepted（[[000292|ADR-000292]]の補完）

## CONTEXT

[[000292|ADR-000292]]でbuildPrompt（検索+リランキング）フェーズにハートビートを導入したが、`missing EndStreamResponse` エラーが依然として発生していた。
...
[[000292|ADR-000292]]のハートビートtickerは`buildPrompt`完了時に`Stop()`されており、以下の区間がカバーされていなかった：
...
- **後方互換性**: 既存のフロントエンド・プロキシ層は[[000292|ADR-000292]]でハートビートスキップ処理を実装済みのため、追加対応不要
...
| テスト名 | 検証内容 |
|---------|---------|
| `TestStream_HeartbeatDuringSlowBuildPrompt` | 既存: 検索+リランキング中のハートビート発行（[[000292|ADR-000292]]） |
...
- [[000289]] nginx `proxy_buffering off` + progressイベント導入
- [[000292]] buildPromptフェーズへのハートビート導入（本ADRの前段対策）
- [[000294]] テスト基盤・DX改善の5フェーズ包括実装
- [[000295]] BFFストリーミングバイパス — io.ReadAll全バッファリングと30秒デッドラインの修正
- [[000297]] フロントエンド progress イベントのフォールスルー修正 — \"searchinggenerating\" 混入の解消
