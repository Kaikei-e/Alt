---
title: "pre-processor: context canceled エラーの防御的修正"
date: 2026-01-25
status: accepted
tags: [docker, testing]
affected_services: [pre-processor]
aliases: [ADR-147, ADR-000147]
---
...
[[000146|ADR 146]] は `Client.Timeout exceeded while awaiting headers` (タイムアウト問題) を修正した。本 ADR が扱う `context canceled` は別のエラーであり、[[000146|ADR 146]] では解決されていない。

| エラー | Go の型 | 原因 |
|--------|---------|------|
| `Client.Timeout exceeded` | `context.DeadlineExceeded` | タイムアウト到達 ([[000146|ADR 146]] で修正済み) |
...
| 本修正の対象 | **対象** | 対象外 ([[000146|ADR 146]] で対応済み) |
