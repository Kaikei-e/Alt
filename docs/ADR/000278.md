---
title: Lighthouse Performance 改善 Phase 2 — SSR 有効化による LCP 最適化
date: 2026-02-27
status: accepted
tags: [connect-rpc, frontend, rss]
affected_services: [alt-frontend-sv, nginx]
aliases: [ADR-278, ADR-000278]
---
...
[[000277|ADR-277]] で `fetchpriority="high"`, `loading="eager"`, nginx gzip/cache, カラーコントラスト修正を適用したが、Lighthouse 再計測の結果 Performance スコアが改善されなかった。
...
[[000277|ADR-277]] で適用した `fetchpriority="high"` と `loading="eager"` は正しく動作していたが、**画像 URL が初期 HTML に存在しないため、ブラウザがリソースを発見できない**。これが Phase 1 の修正が効かなかった根本原因。
...
### カラーコントラスト修正の不足（[[000277|ADR-277]] 残存）

[[000277|ADR-277]] で適用した `bg-[rgba(255,255,255,0.15)]` は、Lighthouse が backdrop-blur を無視してボタン単体の色で計算するため、コントラスト比 1.65 で WCAG AA (4.5:1) 未達だった。
