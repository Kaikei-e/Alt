---
title: "フロントエンド progress イベントのフォールスルー修正 — \"searchinggenerating\" 混入の解消"
date: 2026-02-28
status: accepted
tags: [bugfix, rag]
affected_services: [alt-backend, alt-frontend-sv, nginx, rag-orchestrator]
aliases: [ADR-297, ADR-000297]
---
# ADR-000297: フロントエンド progress イベントのフォールスルー修正 — "searchinggenerating" 混入の解消

## ADR's STATUS

Accepted（[[000295|ADR-000295]]の後続修正）

## CONTEXT

[[000289|ADR-289]]〜295でRAGストリーミングパイプライン全体（nginx → BFF → rag-orchestrator）の修正が完了し、ストリーミング自体は正常に稼働するようになった。しかし、ユーザーの回答テキストの先頭に `searchinggenerating` という文字列が必ず付与される問題が発生していた。
...
- [[000289]] nginx `proxy_buffering off` + progress イベント導入
- [[000292]] Augur ストリーミング30秒タイムアウト修正
- [[000293]] ハートビートをLLMストリーミングフェーズに拡張
- [[000295]] BFF ストリーミングバイパス修正
