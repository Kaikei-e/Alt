---
title: フィード除外フィルタ — excludeClause ヘルパー関数への DRY リファクタリング
date: 2026-02-21
status: accepted
tags: [backend, clean-architecture, refactoring, rss, security]
affected_services: [alt-backend, alt-frontend-sv]
aliases: [ADR-260, ADR-000260]
---
...
[[000256|ADR-000256]] でフィードソース除外フィルタを導入した。バックエンドの `fetch_feed_driver.go` では、除外対象の `feed_link_id` を受け取り `NOT EXISTS` + `split_part` によるドメインフォールバックで除外する SQL 句を構築している。
