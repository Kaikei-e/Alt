---
title: プレゼンテーション層 UUID 生成による Svelte keying ID のドメイン分離
date: 2026-02-13
status: accepted
tags: [ai, api, backend, clean-architecture]
affected_services: [alt-backend]
aliases: [ADR-238, ADR-000238]
---
...
[[000237|ADR-000237]] の第1版では、Favorites ページの Svelte reconcile クラッシュ（重複キー問題）を修正するために、DB の主キー `feeds.id` を `domain.FeedItem.FeedID` としてドメイン層に追加した。
...
- [[000237]] Favorites 重複キーによる Svelte reconcile クラッシュ修正（本 ADR の前段）
