---
title: "rask-log-forwarder: trace_id/span_id消失バグの修正"
date: 2026-01-19
status: accepted
tags: [bugfix, docker, logging, monitoring, testing]
affected_services: [auth-hub, rask-log-aggregator, rask-log-forwarder]
aliases: [ADR-128, ADR-000128]
---
...
[[000122|ADR-122]]でGoサービスのstdoutログにtrace_id/span_idを出力する`TraceContextHandler`を実装したが、ClickHouseに保存されるログのTraceId/SpanIdがすべてゼロになる問題が発生した。
...
- [[000122]] Grafana Trace/Log表示問題の修正 (TraceContextHandler導入)
- [[000129]] rask-log-aggregator: OTLP HTTP専用ポート (4318) の追加
- [[000130]] rask-log-aggregator: otel_traces INSERT時のカラム競合エラー修正
