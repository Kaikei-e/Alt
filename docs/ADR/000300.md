---
title: GetFeedIDByURL リグレッション修正 — feed_links.id と feeds.id の混同による FK 違反
date: 2026-02-28
status: accepted
tags: [backend, bugfix, clean-architecture, connect-rpc, rss]
affected_services: [alt-backend, pre-processor]
aliases: [ADR-300, ADR-000300]
---
...
[[000299|ADR-000299]] Fix #3 で `GetFeedIDByURL` のクエリ対象テーブルを `feeds` から `feed_links` に変更した。変更前のクエリはウェブサイト URL が格納されている `feeds.link` を参照しており、RSS URL での検索が常に失敗していた。
...
## APPENDIX
- [[000301]] GetFeedIDByURL の二重用途バグ修正 — 記事URL と RSS ソースURL の分離
...
[[000300|ADR-000300]] の JOIN 修正により `GetFeedIDByURL` は RSS ソース URL（`feed_links.url`）を正しく検索できるようになったが、`SaveArticle` が同じ関数を **記事 URL** で呼び出していることが判明した。
