---
title: Desktop Feed Modal 状態管理レースコンディション修正
date: 2026-01-27
status: accepted
tags: [bugfix, docker, frontend, testing]
affected_services: [alt-frontend-sv]
aliases: [ADR-150, ADR-000150]
---
...
| 操作対象 | 結果 | nextFeedUrl |
|----------|------|-------------|
| 先頭〜中間のフィード | 次のフィードへ移動 | 次のフィードURL |
| 最後のフィード | モーダルを閉じる | `null` |
| 唯一のフィード | モーダルを閉じる | `null` |

### 修正後のフロー

```typescript
// After: URL ベースで安定
const { nextFeedUrl } = feedGridApi.removeFeedByUrl(feedUrl);  // 同期

if (nextFeedUrl === null) {
  isModalOpen = false;  // 最後のフィードまたは全て既読
  selectedFeedUrl = null;
} else {
  selectedFeedUrl = nextFeedUrl;  // 次のフィードへ移動
}
...
function removeFeedByUrl(url: string): RemoveFeedResult {
  ...
  // 最後のアイテムを削除した場合は null を返す（モーダルを閉じる）
  if (wasLastItem) {
    return { nextFeedUrl: null, totalCount: newVisibleFeeds.length };
  }
