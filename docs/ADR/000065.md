---
title: Desktop Feed Details Modal でのキャッシュキー不一致によるコンテンツ表示バグ修正
date: 2026-01-09
status: accepted
tags: [backend, caching, frontend, performance, testing]
affected_services: []
---
...
`src/lib/utils/articlePrefetcher.spec.ts` に以下のテストケースを追加：

| テストケース | 検証内容 |
|-------------|---------|
| `should use normalizedUrl as cache key, not link` | キャッシュキーが `normalizedUrl` であること、API 呼び出しも `normalizedUrl` を使用すること |
| `should separate cache entries for different normalizedUrls` | 異なる `normalizedUrl` のフィードが別々のキャッシュエントリを持つこと |
| `should not return cached content when accessed with link instead of normalizedUrl` | `link` でアクセスしてもキャッシュがヒットしないこと（キーの不一致） |
...
- 型チェック: エラーなし
- ユニットテスト: 106 テストすべてパス（回帰テスト 3 件追加）
