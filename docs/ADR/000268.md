---
title: OGP 画像取得をバックエンド抽出方式へ移行
date: 2026-02-26
status: accepted
tags: [backend, caching, connect-rpc, database, security]
affected_services: [alt-backend, alt-frontend-sv]
aliases: [ADR-268, ADR-000268]
---
...
[[000267|ADR-000267]] で Visual Preview スワイプモードを実装した際、OGP 画像 URL の抽出はフロントエンド側で行っていた。具体的には、`ArticlePrefetcher` が取得した HTML コンテンツを `ThumbnailExtractor`（regex ベース）で解析し、`ThumbnailPrefetcher`（メモリ + localStorage 2 層キャッシュ）で管理する構成だった。
...
| 項目 | [[000267|ADR-000267]] (旧) | [[000268|ADR-000268]] (本 ADR) |
|------|-----------------|---------------------|
