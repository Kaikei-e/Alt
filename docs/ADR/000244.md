---
title: article-sync パイプライン障害の段階的修正
date: 2026-02-15
status: accepted
tags: [caching, clean-architecture, connect-rpc, queue, testing]
affected_services: [alt-backend, auth-hub, pre-processor]
aliases: [ADR-244, ADR-000244]
---
...
- [[000247]] 要約パイプライン修正 — APIモードにおける `ListUnsummarizedArticles` RPC の追加

### article-sync パイプラインの全体像

```
┌─────────────┐    ┌─────────────┐    ┌──────────┐
│pre-processor│───▶│ alt-backend  │───▶│ auth-hub │
│             │    │              │    │          │
│ article-sync│    │ /v1/internal │    │ /internal│
│ (毎時)      │    │ /system-user │    │ /system- │
│             │    │              │    │ user     │
└──────┬──────┘    └──────────────┘    └──────────┘
       │
       │ FetchInoreaderArticles
       │ (pre-processor DB 直接クエリ)
       ▼
┌──────────────┐
│ inoreader_   │
│ articles     │
...
### API モードにおけるリポジトリのデータソース分類

| メソッド | データソース | 理由 |
|---------|-------------|------|
| Create, CheckExists, FindByID | Connect-RPC (alt-backend) | 記事データは alt-backend が管理 |
| FetchInoreaderArticles | DB 直接クエリ (pre-processor DB) | Inoreader テーブルは pre-processor 固有 |
| UpsertArticles | Connect-RPC (alt-backend) | 記事の永続化は alt-backend 経由 |
| FindForSummarization | 未使用 (イベント駆動) | API モードでは Redis Streams 経由 |
