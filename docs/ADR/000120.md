---
title: 未対応サービスへのOTel準拠ロガー導入
date: 2026-01-17
status: accepted
tags: [docker, logging, monitoring, rag, search]
affected_services: [alt-backend, alt-perf, auth-hub, auth-token-manager, news-creator, pre-processor, rag-orchestrator, rask-log-aggregator, recap-evaluator, recap-subworker, search-indexer, tag-generator]
aliases: [ADR-120, ADR-000120]
---
...
[[000098|ADR-98]]で定義されたOpenTelemetry準拠のログ基盤において、一部サービスがOTel対応されていなかった。
...
| recap-evaluator | Python | structlog（OTelなし）、[[000098|ADR 98]]未準拠 | MEDIUM |
...
| `src/recap_evaluator/utils/logging.py` | LoggingInstrumentor統合、[[000098|ADR 98]]コンテキスト追加 |
...
3. **ビジネスコンテキスト**: [[000098|ADR 98]]準拠の `alt.*` 属性が全サービスで利用可能
...
- [[000098]] ClickHouseログ基盤の包括的改善
- [[000099]] ログのビジネスコンテキスト追加（`alt.*`属性定義）
