...
[[000203|ADR-000203]] では news-creator 側のキュー管理を改善した。本 ADR はその上流にあたる recap-worker 側のバッチ送信パラメータを最適化し、タイムアウト超過の根本原因に対処する。
