---
title: Connect-RPC Server Streaming認証におけるUserContext完全性の確保
date: 2025-12-30
status: accepted
tags: [api, authentication, connect-rpc, database, testing]
affected_services: [alt-backend, alt-frontend-sv, auth-hub, auth-token-manager]
...
[[000028|ADR-000028]]で実装したConnect-RPC v2のServer Streaming RPC（`FeedService.StreamFeedStats`）において、以下の認証エラーが発生した：
...
- [[000028]] Buf Connect-RPCの導入とREST APIからの段階的移行
  - Server Streaming RPCの基盤となる決定
  - 本ADRは[[000028|ADR-000028]]の実装過程で発見された問題への対処
...
- [[000038]] REST API から Connect-RPC への完全移行
