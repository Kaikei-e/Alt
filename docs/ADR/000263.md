---
title: Safari iOS 自動ズーム防止 — モバイル検索 input の font-size 修正
date: 2026-02-25
status: accepted
tags: [backend]
affected_services: [alt-backend, alt-frontend-sv]
aliases: [ADR-263, ADR-000263]
---
...
iOS Safari は `font-size` が 16px 未満の `<input>` にフォーカスした際、ビューポートを自動ズームする仕様がある。[[000261|ADR-000261]] / [[000262|ADR-000262]] で導入したモバイル向け Bottom Sheet の検索入力には Tailwind の `text-sm` (14px) が適用されていたため、iOS ユーザーがフィルタ検索を使用するとビューポートが意図せずズームし、UX が悪化していた。
