---
title: マルチ言語サービスにおける ADR 98 ビジネスコンテキストロギングの実装
date: 2026-01-15
status: accepted
tags: [logging]
affected_services: []
...
---
# マルチ言語サービスにおける [[000098|ADR 98]] ビジネスコンテキストロギングの実装
...
[[000098|ADR 98]] で定義されたビジネスコンテキスト属性（`alt.feed.id`, `alt.article.id`, `alt.job.id`, `alt.processing.stage`, `alt.ai.pipeline`）を、Go/Python/Rust で実装された全マイクロサービスに展開する必要があった。
