---
title: 画像プロキシのドメイン許可リストにサブドメインマッチングを導入
date: 2026-03-02
status: accepted
tags: [database, rss, testing]
affected_services: [alt-backend]
aliases: [ADR-308, ADR-000308]
---
...
[[000307|ADR-000307]] で導入した OGP 画像プリフェッチ機構により、フィード収集時に RSS XML から画像 URL を抽出し `feeds.og_image_url` に保存、バックグラウンドジョブで画像をキャッシュする仕組みが稼働した。しかし、画像プロキシ経由での表示時にドメイン許可チェックで拒否されるケースが多発していた。
