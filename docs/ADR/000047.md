---
title: ML モデルアーティファクトの Docker コンテナへの配布戦略
date: 2026-01-03
status: accepted
tags: [ci-cd, docker]
affected_services: []
...
1. **`.dockerignore` での除外**
   - [[000046|ADR-046]] で導入した Docker ビルド最適化により、ML モデルアーティファクトがイメージから除外されていた
...
[[000046|ADR-046]] で確立したビルド最適化を維持しながら、モデル配布の問題を解決する。
...
2. **ビルド最適化の維持**
   - [[000046|ADR-046]] で達成したビルド時間短縮を維持
   - イメージサイズは小さいまま
