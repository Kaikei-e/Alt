---
title: tag-generator API モード移行に伴うトランザクション管理コードの修正
date: 2026-02-26
status: accepted
tags: [backend, connect-rpc]
affected_services: [alt-backend, tag-generator]
aliases: [ADR-266, ADR-000266]
---
...
[[000241|ADR-000241]] Phase 3 で Shared Database アンチパターン解消のため、tag-generator を直接 DB 接続から Connect-RPC API モードに移行した。API モードでは `ConnectArticleFetcher` / `ConnectTagInserter` ドライバが alt-backend 経由でデータアクセスを行い、DB コネクションは不要となる。
...
- レガシー DB モードのコードパスがそのまま残るため、完全な API モード移行後にはデッドコードとなる。[[000241|ADR-000241]] の最終フェーズでの削除を推奨
...
- [[000241]] Shared Database アンチパターン解消（Connect-RPC API モード移行）
