---
title: "Swipe モード向け Filter & Sort Bottom Sheet"
date: 2026-02-25
status: accepted
tags: [backend, connect-rpc, frontend, refactoring, rss]
affected_services: [alt-backend, alt-frontend-sv]
aliases: [ADR-262, ADR-000262]
---
...
Swipe モード (`/(app)/feeds/swipe`) にはフィード一覧のフィルタリング機能が存在しなかった。通常の Feeds ページには [[000261|ADR-000261]] で `MobileFeedExcludeFilter` (Filter Chip + Bottom Sheet) が実装済みだが、Swipe モードは独立した `SwipeFeedScreen` コンポーネントで構成されており、フィルタ UI が接続されていなかった。

多数のソースを購読しているユーザーが、Swipe ブラウジング中に特定ソースを一時除外したいニーズに対応する。

### 前提条件

- Connect-RPC API の `excludeFeedLinkId` パラメータは [[000256|ADR-000256]] で実装済み
- `getFeedsWithCursorClient()` は既に第 3 引数として `excludeFeedLinkId` を受け付ける
- `filterSources()` / `extractDomain()` は [[000261|ADR-000261]] で `feed-source-filter.ts` に共有ユーティリティとして抽出済み
...
- v1 は [[000261|ADR-000261]] と同様に単一ソース除外のみ。複数ソース除外は将来の拡張
...
- [[000256]] フィードソース除外フィルタ (Connect-RPC API + デスクトップ UI)
...
- [[000263]] Safari iOS 自動ズーム防止 — モバイル検索 input の font-size 修正
- [[000264]] Summary Request FIFO→LIFO — Swipe Feed 体感レイテンシ最適化
