---
title: "SaveArticleSummary UUID エラー修正 — `user_id` パススルー"
date: 2026-02-15
status: accepted
tags: [ai, api, database, frontend, testing]
affected_services: [alt-backend, news-creator, pre-processor]
aliases: [ADR-248, ADR-000248]
---
...
[[000247|ADR-000247]] で要約ポーリングパイプラインを復旧した後、news-creator による要約生成自体は成功していたが、**alt-backend での保存操作がすべて失敗**していた。
...
- [[000247|ADR-000247]] デプロイ後、要約保存操作の 100% が失敗
...
pre-processor は `ListUnsummarizedArticles` レスポンス（[[000247|ADR-000247]] で追加）から各記事の `user_id` を既に受信している。この修正では、`SaveArticleSummaryRequest` proto メッセージを通じてこの値を渡すことで、ハードコードされた値を排除する。
...
- [[000247]] 要約パイプライン修正（`ListUnsummarizedArticles` RPC）
