---
title: 403エラーに対する指数バックオフリトライと自動無効化
date: 2026-02-28
status: accepted
tags: [backend, testing]
affected_services: [alt-backend]
aliases: [ADR-296, ADR-000296]
---
# ADR-000296: 403エラーに対する指数バックオフリトライと自動無効化

## ADR's STATUS

Accepted（[[000290|ADR-000290]]の後続改善）

## CONTEXT

[[000290|ADR-000290]] で HEAD リクエストを廃止し、GET リクエストのみでフィード収集を行うように変更した。その際、403 を `isPersistentError` から除外した（HEAD 特有の誤検知を回避するため）。
