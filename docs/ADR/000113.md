---
title: Recap Job ステータス不整合の修正
date: 2026-01-17
status: accepted
tags: [database, recap, testing]
affected_services: [recap-worker]
...
Job Status Dashboard（[[000109|ADR-109]]）で、一部のジョブが `status = 'pending'` かつ `last_stage = 'persist'` という矛盾した状態で表示される問題が報告された。
