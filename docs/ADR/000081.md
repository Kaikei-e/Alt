---
title: recap-worker パイプラインのバッチ API 完全移行
date: 2026-01-13
status: accepted
tags: [performance]
affected_services: [news-creator, recap-worker]
...
[[000080|ADR-080]] で news-creator にバッチ API（`/v1/summary/generate/batch`）を実装したが、recap-worker のパイプラインは依然として個別の API 呼び出しを行っていた。
...
| 順次処理 | 各ジャンルの待機時間が累積 |
| バッチ API 未活用 | [[000080|ADR-080]] の改善効果が未発揮 |
...
- [[000080]] チャッティマイクロサービス改善（バッチ API 実装）
