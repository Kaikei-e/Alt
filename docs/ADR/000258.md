---
title: フィードソース除外フィルタの (app)/feeds ページへの移植
date: 2026-02-21
status: accepted
tags: [bugfix, frontend, testing]
affected_services: [alt-frontend-sv]
aliases: [ADR-258, ADR-000258]
---
...
[[000256|ADR-000256]] でフィードソース除外フィルタを導入し、[[000257|ADR-000257]] で `listSubscriptions` のツリーシェイクバグを修正した。しかし、ユーザーが実際にアクセスする `/sv/feeds` ページでは依然として除外フィルタが機能しない問題が継続していた。

### 原因分析

`hooks.server.ts` が `/sv/desktop/feeds` から `/sv/feeds` への 301 リダイレクトを発行しているため、ユーザーが実際にアクセスするのは `(app)/feeds/+page.svelte` である。

[[000256|ADR-000256]] の実装は `desktop/feeds/+page.svelte` にのみ配線されており、`(app)/feeds/+page.svelte` には以下が欠落していた:
...
既存の `FeedFilters`, `FeedGrid`, `FeedSourceExcludeFilter` コンポーネントや API クライアント層は変更不要。[[000256|ADR-256]] / [[000257|ADR-257]] で正しく実装済みであり、ページコンポーネントからの配線のみが欠落していた。
...
| ファイル | 理由 |
|---------|------|
| `FeedFilters.svelte` | [[000256|ADR-256]] で `feedSources`, `excludedFeedLinkId` props を既にサポート済み |
| `FeedGrid.svelte` | [[000256|ADR-256]] で `excludedFeedLinkId` prop と `$effect` によるリロード制御を実装済み |
| `FeedSourceExcludeFilter.svelte` | 変更不要 |
| `$lib/api/client/feeds.ts` | [[000257|ADR-257]] で修正済み |
| `$lib/connect/feeds/listing.ts` | [[000256|ADR-256]] で対応済み |
| バックエンド全体 | [[000256|ADR-256]] で対応済み |
...
- 既存コンポーネント・API 層は一切変更不要。[[000256|ADR-256]] / [[000257|ADR-257]] の設計が正しかったことを確認
...
- [[000256]] フィードソース除外フィルタ (Feeds ページ)
- [[000257]] FeedSourceExcludeFilter の listSubscriptions リクエスト未送信バグ修正
- [[000259]] ListSubscriptions 500 エラー修正 (subscription_driver.go SQL カラム不整合)
