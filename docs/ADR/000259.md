---
title: ListSubscriptions 500 エラー修正 (subscription_driver.go SQL カラム不整合)
date: 2026-02-21
status: accepted
tags: [backend, bugfix, clean-architecture, database, rss]
affected_services: [alt-backend, alt-frontend-sv]
aliases: [ADR-259, ADR-000259]
---
...
[[000256|ADR-000256]] でフィードソース除外フィルタを導入し、[[000257|ADR-000257]] でツリーシェイクバグを修正、[[000258|ADR-000258]] で `(app)/feeds/+page.svelte` への配線を完了した。これにより `ListSubscriptions` RPC が正しく発行されるようになったが、バックエンドが 500 Internal Server Error を返却し、フィードソース一覧を取得できない状態が継続していた。
