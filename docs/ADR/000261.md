...
[[000256|ADR-000256]] で導入したフィードソース除外フィルタはデスクトップ UI (`/sv/feeds`) のみに展開されていた。デスクトップではオートコンプリート入力 + チップ表示のパターンを使用している。
...
- [[000323]] デスクトップ向け Visual Preview UI の実装
