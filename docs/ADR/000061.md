---
title: フロントエンド専用開発環境（frontend-dev スタック）
date: 2026-01-08
status: accepted
tags: [api, authentication, database, frontend, testing]
affected_services: [alt-backend, alt-frontend-sv, altctl]
...
[[000060|ADR-060]]で導入した `dev` スタックは実際のバックエンドとDBを使用するため、以下の課題があった：
...
- [[000062]] Mock Server モジュール分割とMSW対応
