---
title: "要約パイプライン修正 — APIモードにおける `ListUnsummarizedArticles` RPC の追加"
date: 2026-02-15
status: accepted
tags: [ai, caching, connect-rpc, database, queue]
affected_services: [alt-backend, news-creator, pre-processor, tag-generator]
aliases: [ADR-247, ADR-000247]
---
...
pre-processor を APIモード（[[000246|ADR-000246]]）に移行した後、要約パイプラインが機能しなくなった。データベースに未要約記事が82件存在するにもかかわらず、pre-processor の10秒ポーリングループは常に `processed:0` を返し、news-creator サービスへのリクエストが一切送信されていなかった。
...
- [[000241]] 共有データベース・アンチパターンの解消戦略
- [[000244]] article-sync パイプライン概要とデータソース分類
- [[000245]] 未登録フィードに対する `UpsertArticles` の耐障害性向上
- [[000246]] pre-processor-db 分離 — alt-db 依存の完全排除
- [[000248]] SaveArticleSummary UUID エラー修正 — `user_id` パススルー
- [[000249]] 要約パイプラインのイベント駆動化
- [[000250]] バッチ要約ジョブのカーソルページネーション固着バグの修正
- [[000251]] 要約バッチと品質チェッカーの無限ループ解消
