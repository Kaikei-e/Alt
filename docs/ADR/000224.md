---
title: Svelte 5 $effect 内 isLoading ガード除去による初回ローディングスピナー永続表示バグの修正
date: 2026-02-11
status: accepted
tags: [bugfix]
affected_services: [alt-frontend-sv]
aliases: [ADR-224, ADR-000224]
---
...
[[000221|ADR-221]] で `bind:this` + `$effect` パターンへ移行した後、Desktop 版の複数ページで初回のローディングスピナーが永続表示されるバグが発生した。
...
- 関連 ADR: [[000221|ADR-221]]（`bind:this` + `$effect` パターン移行）
