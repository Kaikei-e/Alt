...
| 関連 ADR | [[000553]], [[000554]], [[000555]], [[000556]] |

## サマリー

Desktop UI の Visual Preview でオンザフライ要約（StreamSummarize）を実行すると、Cloudflare HTTP 524（100秒タイムアウト）が返るか、データがバックエンドで正常に生成されているにもかかわらずフロントエンドにストリーミング表示されない問題が発生した。調査の結果、5 つの独立した根本原因が重畳していることが判明した。(1) alt-backend が REST パスで pre-processor を呼んでおり `processingArticles` ガードで 409 拒否される、(2) Cloudflare の 100 秒アイドルタイムアウト内にレスポンスの最初のバイトが届かない、(3) SvelteKit プロキシが Connect-RPC JSON モードのストリーミングレスポンスを検出できず nginx がバッファする、(4) Cloudflare HTTP/3 と Connect-RPC server streaming の相性問題で QUIC プロトコルエラー、(5) プリエンプションが in-flight Ollama HTTP リクエストを中断できず RT が GPU スロットを取得できない。
...
- 5 つの根本原因が重畳していたため、1 つ修正するたびに次の原因が露出するモグラ叩きになった。事前にエンドツーエンドのストリーミングテストがあれば防げた
- ADR-000164 のプリエンプション設計時に、in-flight HTTP リクエストの中断を考慮すべきだった。`cancel_event` のチェックが「生成前後」のみで、生成中は無力であることが設計レビューで見落とされていた
- Cloudflare が `alt-svc` ヘッダーを保護・上書きすることを事前に検証すべきだった。nginx/SvelteKit で `Alt-Svc: clear` を設定しても無効であることに気づくまで時間を要した
...
5. **プリエンプションは in-flight 操作を中断できなければ無意味**: `cancel_event` のチェックが await の前後のみでは、数十秒の HTTP リクエスト中に RT がブロックされる。`asyncio.wait(FIRST_COMPLETED)` で await 自体を競争させるのが asyncio の標準パターン
6. **Cloudflare は `alt-svc` ヘッダーを保護・上書きする**: オリジンで `Alt-Svc: clear` を設定しても無効。HTTP/3 制御はダッシュボードのゾーン設定のみ。ストリーミング RPC は HTTP/3 と相性が悪いため、ゾーン OFF またはサブドメイン分離が必要

### 組織的教訓

1. **エンドツーエンドのストリーミング E2E テストが必要**: ブラウザ → Cloudflare → nginx → SvelteKit → alt-backend → pre-processor → news-creator の全レイヤーを通したストリーミングテストがあれば、5 つの根本原因を事前に検出できた
2. **プロトコル移行時はシリアライゼーション形式の全パターンをテストすべき**: Connect-RPC は Proto/JSON/gRPC-Web の複数形式をサポートする。移行時に使用中の形式のみテストするのは不十分
3. **CDN の透過的なヘッダー操作を前提とした設計**: Cloudflare 等の CDN はレスポンスヘッダーを書き換える。オリジン側のヘッダー設定が期待通りにクライアントに届くか、実環境で必ず検証すべき
...
| P-6 | nginx streaming location に feeds を追加し StreamSummarize を専用 location で処理 ([[000555]]) | 開発担当者 | 2026-03-24 | **完了** |
| P-7 | Cloudflare HTTP/3 をゾーン全体で OFF に設定（QUIC プロトコルエラー回避） | 開発担当者 | 2026-03-24 | **完了** |
| P-8 | `_generate_with_cancellation` を `asyncio.wait(FIRST_COMPLETED)` で in-flight HTTP 中断可能に修正 ([[000556]]) | 開発担当者 | 2026-03-24 | **完了** |
