...
- The daemon’s `startOAuthServer()` also serves `GET /` and `GET /auth`. Because the endpoint is protected by `INTERNAL_AUTH_TOKEN` and the token is never accepted in the URL (CWE-598), an unauthenticated browser GET answers 401 with a small HTML form: paste the operator token and submit, and the token travels in the POST body before the 302 to Inoreader’s consent page. Scripts can instead send `Authorization: Bearer <INTERNAL_AUTH_TOKEN>` on the GET and follow the `Location` header. Refreshed tokens are stored whenever the callback path hits `?code=...`.
