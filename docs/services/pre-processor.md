...
1. Start the stack (`altctl up` or `docker compose up`). Pre-processor binds to `HTTP_PORT` (default `9200`), logs `Starting background jobs`, and waits for news-creator before launching summarization, quality, and queue loops.
2. Submit jobs:
