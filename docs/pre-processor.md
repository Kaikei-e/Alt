...
flowchart LR
    ...
    API["Echo HTTP API\n(sync/stream/queue/status)"]
    Handler["SummarizeHandler\n(pre/post sanitization)"]
    Queue[(summarize_job_queue)]
    QueueWorker["SummarizeQueueWorker\n(batch=40)"]
    NewsCreator["news-creator\n/summarize & /generate"]
    Postgres[(Postgres\narticles, summaries, summarize_job_queue)]
    ArticleSummarizer["Batch summarization job\n(ticker 10s)"]
    QualityChecker["Quality check job\n(ticker 5m)"]
    ArticleSync["Article sync job\n(once + hourly)"]
    HealthChecker["HealthChecker\n(wait for news-creator models)"]
    AltBackend["alt-backend\n/v1/internal/system-user"]
    Inoreader["Inoreader data"]
    ...
    HealthChecker -->|requires healthy models| NewsCreator
    HealthChecker -->|ready signal| ArticleSummarizer
    HealthChecker -->|ready signal| QualityChecker
    HealthChecker -->|ready signal| QueueWorker
```
...
- **POST /api/v1/summarize/queue** and **GET /api/v1/summarize/status/:job_id**:
  ...
  - Status responses expose `summary`, `error_message`, and the current `status` (`pending`, `running`, `completed`, `failed`).
...
- UUID `job_id`, `article_id`, `status` (`pending`, `running`, `completed`, `failed`), `summary`, `error_message`, `retry_count`, `max_retries`, `started_at`, `completed_at`.
...
| `NEWS_CREATOR_HOST`, `NEWS_CREATOR_API_PATH`, `NEWS_CREATOR_MODEL`, `NEWS_CREATOR_TIMEOUT` | Target news-creator endpoints | `http://news-creator:11434`, `/api/v1/summarize`, `gemma3:4b`, `300s`. |
...
1. Start the stack (`make up` or `docker compose up`). Pre-processor binds to `HTTP_PORT` (default `9200`), logs `Starting background jobs`, and waits for news-creator before launching summarization, quality, and queue loops.
2. Submit jobs:
