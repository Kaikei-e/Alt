...
Here it is scoped to the ten scenarios that genuinely need it, and the other
sixty-one run in parallel.
...
| `02-metrics-shape.hurl` | `tests/health.spec.ts` → "metrics" | Kept at 200 + `text/plain`, deliberately **not** upgraded to `_shared/http.ts`'s `expectPrometheusText`. `Metrics::new` registers into a fresh `Registry` while `render_prometheus` encodes `prometheus::gather()` (the default registry) — the exporter is mute, and asserting `# HELP` would fail. Strengthened instead to "every non-empty line is valid exposition and the body is not a JSON error", which stays true after the registry binding is fixed. |
...
| `05-trigger-and-poll-7days.hurl` | `tests/pipeline.spec.ts` | `retry: 60 / retry-interval: 1000` became `eventually`. Four `jsonpath … exists` checks became real types: `job_id` a UUID, `executed_at` / `window_start` / `window_end` RFC 3339 instants, `genres` a non-empty list of fully-typed genre objects. `[Captures] job_id` is dropped — nothing ever consumed it, and `get_latest_completed_job` gives no way to ask for a specific job. |
| `06-trigger-and-poll-3days.hurl` | `tests/pipeline.spec.ts` | Same drive, parameterised. Upgraded with the assertion the file's own comment claimed but never made: `window_end - window_start` must be exactly 3 days, which is what would catch a `window_days` that stopped propagating. |
