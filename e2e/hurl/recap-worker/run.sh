...
cleanup() {
  if [[ "${KEEP_STACK:-0}" != "1" ]]; then
    ...
    # --profile is required on `down` so anonymous postgres volumes
    # attached to profile-scoped services (recap-db) are removed. Without
    # it, the next run resurrects the old schema + rows and scenario 03
    # sees pre-existing recap_outputs.
    docker compose -f compose/compose.staging.yaml -p alt-staging \
      --profile recap-worker \
...
echo "==> bringing up recap-worker staging slice" >&2
docker compose -f compose/compose.staging.yaml -p alt-staging \
  --profile recap-worker \
