...
// TestRememberDLQWrite_Bounded asserts the awaiting-ack set cannot grow without
// limit while Redis keeps refusing XACKs.
func TestRememberDLQWrite_Bounded(t *testing.T) {
	c := &Consumer{}
	for i := 0; i < dlqAwaitingAckMax*2; i++ {
		c.rememberDLQWrite(string(rune('a')) + string(rune(i)))
	}
	c.dlqMu.Lock()
	n := len(c.dlqAwaitingAck)
	c.dlqMu.Unlock()
	if n > dlqAwaitingAckMax {
		t.Fatalf("dlqAwaitingAck grew to %d, want <= %d", n, dlqAwaitingAckMax)
	}
}
