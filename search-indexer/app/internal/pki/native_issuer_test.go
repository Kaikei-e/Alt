...
func newNativeIssuer(t *testing.T, ca *fakeStepCA) *NativeStepCAIssuer {
	t.Helper()
	url, root := ca.start(t)
	pw := writePasswordFile(t, t.TempDir(), "pki-agent-search-indexer-jwk", string(ca.password), 0o400)
...
func TestNativeStepCAIssuer_Issue_Success(t *testing.T) {
	...
	ca := newFakeStepCA(t, "pki-agent-search-indexer", password)
	iss := newNativeIssuer(t, ca)

	certPEM, keyPEM, err := iss.Issue(context.Background(), "search-indexer", []string{"search-indexer"})
	if err != nil {
		ca.mu.Lock()
	...
	if claims.Issuer != "pki-agent-search-indexer" {
		t.Fatalf("iss=%q", claims.Issuer)
	}
	if claims.Subject != "search-indexer" {
		t.Fatalf("sub=%q", claims.Subject)
	}
	if !audienceHasSign(claims.Audience) {
		t.Fatalf("aud=%v", claims.Audience)
	}
	if len(claims.SANs) == 0 || claims.SANs[0] != "search-indexer" {
	...
	if leaf.Subject.CommonName != "search-indexer" {
...
func TestNativeStepCAIssuer_MintsDistinctOTTs(t *testing.T) {
	ca := newFakeStepCA(t, "pki-agent-search-indexer", []byte("pw-distinct"))
	iss := newNativeIssuer(t, ca)
	if _, _, err := iss.Issue(context.Background(), "search-indexer", []string{"search-indexer"}); err != nil {
		t.Fatal(err)
	}
	if _, _, err := iss.Issue(context.Background(), "search-indexer", []string{"search-indexer"}); err != nil {
		t.Fatal(err)
...
func TestNativeStepCAIssuer_ReusedOTTRejectedByCA(t *testing.T) {
	ca := newFakeStepCA(t, "pki-agent-search-indexer", []byte("pw-reuse"))
	iss := newNativeIssuer(t, ca)
	if _, _, err := iss.Issue(context.Background(), "search-indexer", []string{"search-indexer"}); err != nil {
		t.Fatal(err)
	}
...
func TestNativeStepCAIssuer_ExpiredOTTRejectedByCA(t *testing.T) {
	ca := newFakeStepCA(t, "pki-agent-search-indexer", []byte("pw-exp"))
	iss := newNativeIssuer(t, ca)
	...
	csrPEM := mustCSRPEM(t, "search-indexer")
...
func TestNativeStepCAIssuer_WrongCARoot(t *testing.T) {
	ca := newFakeStepCA(t, "pki-agent-search-indexer", []byte("pw-root"))
	iss := newNativeIssuer(t, ca)
	...
	_, _, err := iss.Issue(context.Background(), "search-indexer", []string{"search-indexer"})
	if err == nil {
...
func TestNativeStepCAIssuer_WrongSubjectRejected(t *testing.T) {
	ca := newFakeStepCA(t, "pki-agent-search-indexer", []byte("pw-sub"))
...
func TestNativeStepCAIssuer_WrongSANRejected(t *testing.T) {
	ca := newFakeStepCA(t, "pki-agent-search-indexer", []byte("pw-san"))
	ca.mutateLeaf = func(c *x509.Certificate) {
...
func TestNativeStepCAIssuer_MalformedResponse(t *testing.T) {
	ca := newFakeStepCA(t, "pki-agent-search-indexer", []byte("pw-malformed"))
...
func TestNativeStepCAIssuer_Timeout(t *testing.T) {
	ca := newFakeStepCA(t, "pki-agent-search-indexer", []byte("pw-timeout"))
	...
	defer cancel()
	start := time.Now()
	_, _, err := iss.Issue(ctx, "search-indexer", []string{"search-indexer"})
	if err == nil {
...
func TestNativeStepCAIssuer_Canceled(t *testing.T) {
	ca := newFakeStepCA(t, "pki-agent-search-indexer", []byte("pw-cancel"))
...
func TestNativeStepCAIssuer_PasswordFileErrors(t *testing.T) {
	ca := newFakeStepCA(t, "pki-agent-search-indexer", []byte("pw-files"))
	url, root := ca.start(t)
	tests := []struct {
		name string
...
func TestNativeStepCAIssuer_DoesNotLogSecrets(t *testing.T) {
	ca := newFakeStepCA(t, "pki-agent-search-indexer", []byte("super-secret-jwk-password"))
	iss := newNativeIssuer(t, ca)
	if _, _, err := iss.Issue(context.Background(), "search-indexer", []string{"search-indexer"}); err != nil {
		t.Fatal(err)
	}
...
func TestNativeStepCAIssuer_RejectsSharedProvisioner(t *testing.T) {
	iss := &NativeStepCAIssuer{Provisioner: "pki-agent", PasswordFile: "/run/secrets/pki-agent-search-indexer-jwk"}
	if _, _, err := iss.Issue(context.Background(), "search-indexer", []string{"search-indexer"}); !errors.Is(err, ErrSharedProvisioner) {
		t.Fatalf("got %v", err)
	}
}
...
func TestNativeStepCAIssuer_RejectsHTTP(t *testing.T) {
	...
	if _, _, err := iss.Issue(context.Background(), "search-indexer", []string{"search-indexer"}); !errors.Is(err, ErrNotHTTPS) {
		t.Fatalf("got %v", err)
	}
}

func TestNativeStepCAIssuer_RejectsRedirect(t *testing.T) {
	ca := newFakeStepCA(t, "pki-agent-search-indexer", []byte("pw-redirect"))
	ca.redirect = true
	iss := newNativeIssuer(t, ca)
	_, _, err := iss.Issue(context.Background(), "search-indexer", []string{"search-indexer"})
...
func TestNativeStepCAIssuer_PasswordTooLarge(t *testing.T) {
	ca := newFakeStepCA(t, "pki-agent-search-indexer", []byte("pw-cap"))
	url, root := ca.start(t)
	path := filepath.Join(t.TempDir(), "huge")
	if err := os.WriteFile(path, bytes.Repeat([]byte("a"), maxPasswordBytes+1), 0o400); err != nil {
		t.Fatal(err)
...
func TestNativeStepCAIssuer_AcceptsWorldReadablePassword(t *testing.T) {
	ca := newFakeStepCA(t, "pki-agent-search-indexer", []byte("docker-secret"))
	url, root := ca.start(t)
	pw := writePasswordFile(t, t.TempDir(), "pki-agent-search-indexer-jwk", string(ca.password), 0o444)
	iss := &NativeStepCAIssuer{
		CAURL: url, RootFile: root,
		Provisioner:  ca.provisioner,
		PasswordFile: pw,
...
func TestNativeStepCAIssuer_ProvisionerPageCap(t *testing.T) {
	ca := newFakeStepCA(t, "pki-agent-search-indexer", []byte("pw-pages"))
...
func TestNativeStepCAIssuer_ResponseTooLarge(t *testing.T) {
	ca := newFakeStepCA(t, "pki-agent-search-indexer", []byte("pw-huge-body"))
...
func TestNativeStepCAIssuer_CAErrorStatusOnly(t *testing.T) {
	...
	ca := newFakeStepCA(t, "pki-agent-search-indexer", []byte("pw-leak"))
...
func TestNativeStepCAIssuer_ExtraDNSSANRejected(t *testing.T) {
	ca := newFakeStepCA(t, "pki-agent-search-indexer", []byte("pw-extra-san"))
	ca.mutateLeaf = func(c *x509.Certificate) {
...
func TestNativeStepCAIssuer_TypedSANSet(t *testing.T) {
	ca := newFakeStepCA(t, "pki-agent-search-indexer", []byte("pw-typed-san"))
	iss := newNativeIssuer(t, ca)
	sans := []string{"search-indexer", "127.0.0.1", "https://alt.example/svc"}
	certPEM, _, err := iss.Issue(context.Background(), "search-indexer", sans)
	if err != nil {
		t.Fatal(err)
	}
	block, _ := pem.Decode(certPEM)
	...
	if len(leaf.DNSNames) != 1 || leaf.DNSNames[0] != "search-indexer" {
...
func TestNativeStepCAIssuer_WrongLeafPublicKeyRejected(t *testing.T) {
	ca := newFakeStepCA(t, "pki-agent-search-indexer", []byte("pw-wrong-key"))
...
func TestNativeStepCAIssuer_RekeyUsesClientCert(t *testing.T) {
	ca := newFakeStepCA(t, "pki-agent-search-indexer", []byte("pw-rekey"))
	iss := newNativeIssuer(t, ca)
	certPEM, keyPEM, err := iss.Issue(context.Background(), "search-indexer", []string{"search-indexer"})
	if err != nil {
		t.Fatal(err)
	}
	newCert, newKey, err := iss.Rekey(context.Background(), certPEM, keyPEM, "search-indexer", []string{"search-indexer"})
	if err != nil {
		t.Fatal(err)
	}
