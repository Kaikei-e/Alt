...
func TestLoadConfig_EmptyEnrollmentFails(t *testing.T) {
	...
	if _, err := LoadConfig("search-indexer"); err == nil {
...
func TestLoadConfig_ExactProvisionerAndPasswordBasename(t *testing.T) {
	...
	t.Setenv("STEP_CA_PROVISIONER", "pki-agent-search-indexer-extra")
	t.Setenv("STEP_CA_PROVISIONER_PASSWORD_FILE", "/run/secrets/pki-agent-search-indexer-extra-jwk")
	_, err := LoadConfig("search-indexer")
	if err == nil {
...
func TestLoadConfig_TempDirAllowedWhenBasenameMatches(t *testing.T) {
	t.Setenv("PKI_ENROLLMENT", ModeEnabled)
	t.Setenv("CERT_SUBJECT", "search-indexer")
	path := filepath.Join(t.TempDir(), "pki-agent-search-indexer-jwk")
	t.Setenv("STEP_CA_PROVISIONER_PASSWORD_FILE", path)
	c, err := LoadConfig("search-indexer")
	if err != nil {
		t.Fatal(err)
	}
...
func TestNativeStepCAIssuer_RejectsWrongP2CBeforeDecrypt(t *testing.T) {
	ca := newFakeStepCA(t, "pki-agent-search-indexer", []byte("pw-p2c"))
...
func TestNativeStepCAIssuer_RequiresDualEKU(t *testing.T) {
	ca := newFakeStepCA(t, "pki-agent-search-indexer", []byte("pw-eku"))
...
func TestCertFile_WriteRespectsCanceledContext(t *testing.T) {
	...
	cert, key := newSelfSignedPEM(t, "search-indexer", nb, nb.Add(time.Hour))
...
func TestNativeStepCAIssuer_CARootSymlinkRejected(t *testing.T) {
	ca := newFakeStepCA(t, "pki-agent-search-indexer", []byte("pw-rootlink"))
	iss := newNativeIssuer(t, ca)
	dir := t.TempDir()
...
func TestCertFile_CertRenameFailureRollsBackKey(t *testing.T) {
	...
	cf := &CertFile{CertPath: filepath.Join(dir, "svc-cert.pem"), KeyPath: filepath.Join(dir, "svc-key.pem")}
	nb := time.Now().Add(-time.Minute)
	oldCert, oldKey := newSelfSignedPEM(t, "search-indexer", nb, nb.Add(time.Hour))
	if err := cf.Write(context.Background(), oldCert, oldKey); err != nil {
		t.Fatal(err)
	}
	newCert, newKey := newSelfSignedPEM(t, "search-indexer", nb, nb.Add(2*time.Hour))
...
func TestCertFile_CrashBetweenRenames_LoadCorruptAndTickReissues(t *testing.T) {
	...
	newCert, newKey := newSelfSignedPEM(t, "search-indexer", nb, nb.Add(48*time.Hour))
...
func TestTick_CrashKeepsTLSUtilLastGood(t *testing.T) {
	...
	first, err := cfg.GetCertificate(&tls.ClientHelloInfo{ServerName: "search-indexer"})
	if err != nil {
		t.Fatal(err)
	}
	newCert, newKey := newSelfSignedPEM(t, "search-indexer", nb, nb.Add(2*time.Hour))
	...
	files.afterKeyRename = nil
	second, err := cfg.GetCertificate(&tls.ClientHelloInfo{ServerName: "search-indexer"})
	if err != nil {
		t.Fatal(err)
	}
