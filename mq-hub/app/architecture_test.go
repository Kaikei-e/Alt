	...
	err := filepath.WalkDir(root, func(path string, d fs.DirEntry, err error) error {
		...
		for _, imp := range node.Imports {
			...
			if strings.HasPrefix(slashPath, "driver/") {
				if importPath == "mq-hub/domain" || strings.HasSuffix(importPath, "/domain") {
					t.Errorf("architecture violation: driver file %s imports domain (%s)", slashPath, importPath)
				}
				if importPath == "mq-hub/port" || strings.HasSuffix(importPath, "/port") {
					t.Errorf("architecture violation: driver file %s imports port (%s)", slashPath, importPath)
				}
				if strings.HasPrefix(importPath, "mq-hub/usecase") ||
					strings.HasPrefix(importPath, "mq-hub/gateway") ||
					strings.HasPrefix(importPath, "mq-hub/connect") {
					t.Errorf("architecture violation: driver file %s imports upward layer (%s)", slashPath, importPath)
				}
			}

			// Rule: port must not import driver, gateway, usecase, or connect
			if strings.HasPrefix(slashPath, "port/") {
				if strings.HasPrefix(importPath, "mq-hub/driver") ||
					strings.HasPrefix(importPath, "mq-hub/gateway") ||
					strings.HasPrefix(importPath, "mq-hub/usecase") ||
					strings.HasPrefix(importPath, "mq-hub/connect") {
					t.Errorf("architecture violation: port file %s imports forbidden layer (%s)", slashPath, importPath)
				}
			}

			// Rule: usecase must not import driver, gateway, or connect
			...
			if strings.HasPrefix(slashPath, "domain/") {
				if strings.HasPrefix(importPath, "mq-hub/") {
