...
const (
	...
	// mq-hub specific context key
	EventTypeKey ContextKey = "alt.event.type"
)
...
func (cl *ContextLogger) WithContext(ctx context.Context) *slog.Logger {
	...
	// mq-hub specific: event type
	if eventType := ctx.Value(EventTypeKey); eventType != nil {
		args = append(args, string(EventTypeKey), eventType.(string))
	}
...
// WithRequestID adds request ID to context for observability
...
// WithEventType adds event type to context for observability (mq-hub specific)
