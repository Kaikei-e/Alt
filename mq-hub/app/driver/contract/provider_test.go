...
//
// failIfNoPactsFound turns "the Broker held nothing for this consumer" from a
// warning into a failure. Without it the verifier logs `Ignoring no pacts
// error` and reports a green verification of zero interactions, so a Broker
// that never received the pact is indistinguishable from a provider that
// satisfies it. It is off for any consumer whose pact is not published into
// the Broker this verification runs against — see the call sites. The
// file-mode branch always supplies a pact, because the test skips earlier when
// the file is missing, so the flag can only fire against the Broker.
...
func TestVerifyTagGeneratorMqHubMessagePact(t *testing.T) {
	verifyMessagePact(t, "tag-generator", tagGeneratorProviderPact,
		...
		// tag-generator's consumer pacts are generated by its Python suite,
		// which the proto-contract workflow never runs as a consumer leg, so
		// nothing publishes tag-generator-mq-hub.json into that workflow's
		// Broker. Only scripts/pact-check.sh, which does run that leg, puts it
		// in front of this verification.
		false,
	)
}
