#!/bin/sh
# Atlas Migration Script for Pre-Processor DB
...
check_requirements() {
    ...
    if [ -z "$DATABASE_URL" ] && [ -n "${PP_DB_HOST:-}" ]; then
        log_info "Constructing DATABASE_URL from environment variables..."
        local user="${PP_DB_USER:-postgres}"
        local name="${PP_DB_NAME:-postgres}"
        local port="${PP_DB_PORT:-5432}"
        local password=""
        if [ -n "${PP_DB_PASSWORD_FILE:-}" ] && [ -f "$PP_DB_PASSWORD_FILE" ]; then
            password=$(cat "$PP_DB_PASSWORD_FILE" | tr -d '\n')
        else
            password="${PP_DB_PASSWORD:-}"
        fi
        local encoded_password
        encoded_password=$(urlencode "$password")
        DATABASE_URL="postgres://$user:$encoded_password@$PP_DB_HOST:$port/$name?sslmode=disable&search_path=public"
        export DATABASE_URL
    fi
...
main() {
    ...
    log_info "Atlas Migration Manager for Pre-Processor DB"
