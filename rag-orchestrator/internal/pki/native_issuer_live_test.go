...
const (
	...
	liveCAContainer = "alt-pki-native-itest-rag"
	liveCANetwork   = "alt-pki-native-itest-rag-net"
	liveCAHostPort  = "19010"
	liveCAPassword  = "itest-only"
	liveJWKPassword = "itest-jwk-rag-orchestrator"
	liveProvisioner = "pki-agent-rag-orchestrator"
	liveSubject     = "rag-orchestrator"
)
...
//
//	docker network create alt-pki-native-itest-rag-net
//	docker run --rm -d --name alt-pki-native-itest-rag \
//	  --network alt-pki-native-itest-rag-net \
//	  -p 127.0.0.1:19010:9000 \
...
//	  -e DOCKER_STEPCA_INIT_WITH_CA_URL=https://127.0.0.1:19010 \
...
func TestNativeStepCAIssuer_DisposableLiveCA(t *testing.T) {
	...
	passwordFile := writePasswordFile(t, tmp, "pki-agent-rag-orchestrator-jwk", liveJWKPassword, 0o400)
...
printf '%%s\n' %q > /tmp/pki-agent-rag-orchestrator-jwk
chmod 400 /tmp/pki-agent-rag-orchestrator-jwk
step ca provisioner add %q --type JWK --create \
  --password-file /tmp/pki-agent-rag-orchestrator-jwk \
