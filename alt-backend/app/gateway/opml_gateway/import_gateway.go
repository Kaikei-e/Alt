...
func (g *ImportGateway) RegisterFeedLinkBulk(ctx context.Context, urls []string) (*domain.OPMLImportResult, error) {
	...
	for _, rawURL := range urls {
		...
		existingID, _ := g.altDB.FetchFeedLinkIDByURL(ctx, trimmed)
		if existingID != nil {
			result.Skipped++
			continue
		}
		...
		err = g.altDB.RegisterRSSFeedLink(ctx, trimmed)
		if err != nil {
			logger.Logger.WarnContext(ctx, "OPML import: failed to register feed link", "url", trimmed, "error", err)
			result.Failed++
			result.FailedURLs = append(result.FailedURLs, trimmed)
			continue
		}
