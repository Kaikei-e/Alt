...
// resolveCoordinationRedisPassword resolves the host-rate-limiter arbiter's
// auth password, but only when coordination is actually configured
// (coordinationRedisURL set). A binary with no coordination story at all, or
// one running the explicit local mode, must not emit redis_auth_disabled for
// a capability it never asked for.
func resolveCoordinationRedisPassword(coordinationRedisURL string) (string, error) {
