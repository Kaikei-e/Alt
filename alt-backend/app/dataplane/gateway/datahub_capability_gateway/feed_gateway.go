...
func (g *FeedGateway) ListByFeedLinkID(ctx context.Context, feedLinkID uuid.UUID) ([]*domain.FeedRow, error) {
	...
	return feedRowsFromPageRows(rows), nil
}
...
func (g *FeedGateway) SearchByTitle(ctx context.Context, query, userID string) ([]*domain.FeedRow, error) {
	...
	return feedRowsFromItems(items), nil
}
...
func (g *FeedGateway) GetInoreaderSummariesByURLs(ctx context.Context, urls []string) ([]*domain.InoreaderSummary, error) {
	...
	return inoreaderSummariesFromDriver(rows), nil
}
...
func feedsToDriverFeeds(feeds []domain.FeedRegistration) []models.Feed {
...
func feedRegistrationResultsFromDriver(rows []alt_db.FeedRegistrationResult) []domain.FeedRegistrationResult {
...
func feedRowsFromPageRows(rows []*alt_db.FeedPageRow) []*domain.FeedRow {
	out := make([]*domain.FeedRow, 0, len(rows))
	for _, r := range rows {
...
func feedRowsFromItems(items []*domain.FeedItem) []*domain.FeedRow {
...
func inoreaderSummariesFromDriver(rows []*models.InoreaderSummary) []*domain.InoreaderSummary {
