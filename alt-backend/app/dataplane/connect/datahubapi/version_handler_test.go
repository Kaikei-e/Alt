...
func versionHandler(t *testing.T, sv *fakeSummaryVersionPort, tsv *fakeTagSetVersionPort, stats *fakeStatsPort) *Handler {
	t.Helper()
	if sv == nil {
	...
	return NewHandler(nil, nil, nil, nil, nil, &fakeSystemUser{}, &fakeRecentArticles{}, slog.Default(),
		WithVersionAndStatsCapabilities(sv, tsv, stats))
}
...
// TestWithVersionAndStatsCapabilitiesRefusesNil is the rule-8 test for this capability.
//
...
func TestWithVersionAndStatsCapabilitiesRefusesNil(t *testing.T) {
	tests := []struct {
		name  string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Panics(t, func() {
				WithVersionAndStatsCapabilities(tt.sv, tt.tsv, tt.stats)
			})
		})
	}
}
...
func TestMarkSummaryVersionSupersededFirstVersion(t *testing.T) {
	h := versionHandler(t, &fakeSummaryVersionPort{prev: nil}, nil, nil)
...
func TestMarkSummaryVersionSupersededRejectsBadIDs(t *testing.T) {
	h := versionHandler(t, nil, nil, nil)
...
func TestGetSummaryVersionByIDReturnsSupersededVersion(t *testing.T) {
	...
	h := versionHandler(t, &fakeSummaryVersionPort{byID: domain.SummaryVersion{
...
func TestGetSummaryVersionNotFound(t *testing.T) {
	h := versionHandler(t, &fakeSummaryVersionPort{notFound: true}, nil, nil)
...
func TestGetLatestSummaryVersionCarriesNoSupersededBy(t *testing.T) {
	h := versionHandler(t, &fakeSummaryVersionPort{latest: domain.SummaryVersion{
...
func TestCreateTagSetVersionPassesTagsJSONThrough(t *testing.T) {
	tsv := &fakeTagSetVersionPort{}
	h := versionHandler(t, nil, tsv, nil)
...
func TestMarkTagSetVersionSupersededFirstVersion(t *testing.T) {
	h := versionHandler(t, nil, &fakeTagSetVersionPort{prev: nil}, nil)
...
func TestGetTagSetVersionByID(t *testing.T) {
	h := versionHandler(t, nil, &fakeTagSetVersionPort{byID: domain.TagSetVersion{
...
func TestStatsProceduresRequireTheTenant(t *testing.T) {
	h := versionHandler(t, nil, nil, &fakeStatsPort{})
...
func TestGetFeedAmountTakesNoTenant(t *testing.T) {
	h := versionHandler(t, nil, nil, &fakeStatsPort{feedAmount: 42})
...
func TestGetTodayUnreadArticlesCountRequiresSince(t *testing.T) {
	stats := &fakeStatsPort{}
	h := versionHandler(t, nil, nil, stats)
...
func TestStatsErrorsAreInternal(t *testing.T) {
	h := versionHandler(t, nil, nil, &fakeStatsPort{err: errors.New("connection refused")})
