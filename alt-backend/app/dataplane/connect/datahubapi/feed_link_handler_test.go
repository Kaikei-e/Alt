...
type feedFakes struct {
	feedLink     *fakeFeedLinkPort
	availability *fakeFeedLinkAvailabilityPort
	feed         *fakeFeedPort
}

func newFeedHandler(f feedFakes) (*Handler, feedFakes) {
	if f.feedLink == nil {
	...
	WithFeedCapabilities(f.feedLink, f.availability, f.feed)(h)
	return h, f
}

const (
	testFeedUserID           = "11111111-2222-3333-4444-555555555555"
	testRegisteredFeedLinkID = "a1b2c3d4-1111-4111-8111-111111111111"
)
...
func TestWithFeedCapabilitiesRejectsNilCollaborators(t *testing.T) {
	feedLink := &fakeFeedLinkPort{}
	availability := &fakeFeedLinkAvailabilityPort{}
	feed := &fakeFeedPort{}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Panics(t, func() {
				WithFeedCapabilities(tt.feedLink, tt.availability, tt.feed)
			})
		})
	}
}
...
func TestRegisterFeedLinkReportsTheDuplicateRatherThanFailing(t *testing.T) {
	h, f := newFeedHandler(feedFakes{feedLink: &fakeFeedLinkPort{alreadyExisted: true}})
...
func TestResolveFeedLinkIDByURLLeavesTheFieldUnsetOnAMiss(t *testing.T) {
	h, _ := newFeedHandler(feedFakes{feedLink: &fakeFeedLinkPort{resolvedID: nil}})
...
func TestListFeedLinksWithHealthOmitsAvailabilityForNeverPolledLinks(t *testing.T) {
	linkID := uuid.MustParse(testRegisteredFeedLinkID)
	h, _ := newFeedHandler(feedFakes{feedLink: &fakeFeedLinkPort{
		withHealth: []*domain.FeedLinkWithHealth{{
...
func TestRecordFeedLinkFailurePassesTheCallersThresholdThrough(t *testing.T) {
	linkID := uuid.MustParse(testRegisteredFeedLinkID)
	h, f := newFeedHandler(feedFakes{availability: &fakeFeedLinkAvailabilityPort{
...
func TestGetSingleFeedLeavesTheFieldUnsetOnAnEmptyTable(t *testing.T) {
	h, _ := newFeedHandler(feedFakes{feed: &fakeFeedPort{single: nil}})
...
func TestGetRandomFeedLeavesTheFieldUnsetWhenNothingIsTagged(t *testing.T) {
	h, _ := newFeedHandler(feedFakes{feed: &fakeFeedPort{random: nil}})
...
func TestListFeedsLimitLeavesAZeroPubDateUnset(t *testing.T) {
	h, _ := newFeedHandler(feedFakes{feed: &fakeFeedPort{rows: []*domain.FeedRow{{
...
func TestFeedReadsSurfaceAsInternalRatherThanAsAnEmptyList(t *testing.T) {
	h, _ := newFeedHandler(feedFakes{feed: &fakeFeedPort{err: errors.New("connection reset")}})
