...
func newReadStateHandler(readState *fakeReadStatePort, tagRead *fakeTagReadPort) (*Handler, *fakeReadStatePort, *fakeTagReadPort) {
	if readState == nil {
	...
	WithReadStateAndTagCapabilities(readState, tagRead)(h)
	...
	for _, tt := range tests {
		for _, absence := range absences {
			t.Run(tt.name+"/"+absence.name, func(t *testing.T) {
				h, _, _ := newReadStateHandler(&fakeReadStatePort{err: absence.err}, nil)
		...
		t.Run(tt.name+"/database failure is not NotFound", func(t *testing.T) {
			h, _, _ := newReadStateHandler(&fakeReadStatePort{err: errors.New("connection refused")}, nil)
			err := tt.invoke(h, context.Background())
			require.Error(t, err)
...
func TestReadState_RequiresUserID(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			h, readState, tagRead := newReadStateHandler(nil, nil)
			err := tt.invoke(h, context.Background())
			require.Error(t, err)
...
func TestReadState_RejectsMalformedUserID(t *testing.T) {
	h, readState, _ := newReadStateHandler(nil, nil)
...
func TestGetAllReadFeedIDs_WithoutSince_PassesNilSince(t *testing.T) {
	feedID1 := uuid.New()
	h, readState, _ := newReadStateHandler(&fakeReadStatePort{
...
func TestListSubscriptions_OmitsSubscribedAtForUnfollowedLinks(t *testing.T) {
	...
	h, _, _ := newReadStateHandler(&fakeReadStatePort{
		subscriptions: []*domain.FeedSource{
...
func TestGetArticleTags_UntaggedIsEmptyNotNotFound(t *testing.T) {
	h, _, _ := newReadStateHandler(nil, &fakeTagReadPort{tags: nil})
...
func TestFeedTagsToProto_LeavesUnsetTimestampsAbsent(t *testing.T) {
	...
	h, _, _ := newReadStateHandler(nil, &fakeTagReadPort{
...
func TestGetFeedTags_LimitAndCursor(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			h, _, tagRead := newReadStateHandler(nil, nil)
...
func TestGetTagArticleCounts_ForwardsWindowAndTenant(t *testing.T) {
	...
	h, _, tagRead := newReadStateHandler(nil, &fakeTagReadPort{
...
// TestWithReadStateAndTagCapabilities_PanicsOnNil is CLAUDE.md rule 8: an unwired
...
func TestWithReadStateAndTagCapabilities_PanicsOnNil(t *testing.T) {
	assert.Panics(t, func() { WithReadStateAndTagCapabilities(nil, &fakeTagReadPort{}) })
	assert.Panics(t, func() { WithReadStateAndTagCapabilities(&fakeReadStatePort{}, nil) })
}
