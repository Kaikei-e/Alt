...
type fakeOgImagePort struct {
	...
	// SaveArticleHead is catalog §2.B, part of the article write capability. It
...
// newMediaHandler builds a Handler with only the media and operational collaborators wired.
// The catalog ports stay nil: this file covers procedures that do not touch
// them, and leaving them out keeps a failure here attributable.
func newMediaHandler(f mediaFakes) (*Handler, mediaFakes) {
	if f.outbox == nil {
...
// TestWithMediaCapabilitiesRejectsNilCollaborators: these are not features
...
func TestWithMediaCapabilitiesRejectsNilCollaborators(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Panics(t, func() {
				WithMediaCapabilities(tt.outbox, tt.ogImage, tt.cache, tt.scraping, tt.auto)
			})
		})
	}
}
...
func TestClaimOutboxBatchReportsProcessing(t *testing.T) {
	h, f := newMediaHandler(mediaFakes{outbox: &fakeOutboxPort{claimed: []domain.OutboxEvent{
...
func TestClaimOutboxBatchDefaultsUnsetLimit(t *testing.T) {
	h, f := newMediaHandler(mediaFakes{})
...
func TestPruneOutboxEventsPassesCallerWindow(t *testing.T) {
	h, f := newMediaHandler(mediaFakes{outbox: &fakeOutboxPort{pruneCount: 12}})
...
func TestGetArticleHeadLeavesFieldUnsetOnMiss(t *testing.T) {
	h, _ := newMediaHandler(mediaFakes{ogImage: &fakeOgImagePort{head: nil}})
...
func TestGetArticleHeadReturnsStoredRow(t *testing.T) {
	h, _ := newMediaHandler(mediaFakes{ogImage: &fakeOgImagePort{
...
func TestGetArticleHeadDoesNotLeakDriverErrorAsMiss(t *testing.T) {
	h, _ := newMediaHandler(mediaFakes{ogImage: &fakeOgImagePort{headErr: errors.New("pool exhausted")}})
...
func TestGetImageProxyCacheLeavesFieldUnsetOnMiss(t *testing.T) {
	h, _ := newMediaHandler(mediaFakes{cache: &fakeImageCachePort{entry: nil}})
...
func TestGetScrapingDomainByDomainLeavesFieldUnsetForUnknownHost(t *testing.T) {
	h, _ := newMediaHandler(mediaFakes{scraping: &fakeScrapingPort{byDomain: nil}})
