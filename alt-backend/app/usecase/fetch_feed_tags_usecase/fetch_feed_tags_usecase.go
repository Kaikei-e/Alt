...
func (u *FetchFeedTagsUsecase) Execute(ctx context.Context, feedURL string, cursor *time.Time, limit int) ([]*domain.FeedTag, error) {
	...
	logger.Logger.Info("fetching feed tags", "feedURL", feedURL, "cursor", cursor, "limit", limit)
	...
	if err != nil {
		logger.Logger.Error("failed to fetch feed tags", "error", err, "feedID", feedID, "limit", limit)
		return nil, err
	}
