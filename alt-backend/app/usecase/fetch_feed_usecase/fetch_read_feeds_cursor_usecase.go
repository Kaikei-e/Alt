...
func (u *FetchReadFeedsListCursorUsecase) Execute(ctx context.Context, cursor *time.Time, limit int) ([]*domain.FeedItem, error) {
	...
	if err != nil {
		logger.Logger.Error("failed to fetch read feeds with cursor", "error", err, "cursor", cursor, "limit", limit)
		return nil, err
	}

	logger.Logger.Info("successfully fetched read feeds with cursor", "count", len(feeds))
	return feeds, nil
}
