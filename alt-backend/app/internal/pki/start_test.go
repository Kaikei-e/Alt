...
func TestStart_DisabledLogs(t *testing.T) {
	...
	h, err := Start(context.Background(), log, "alt-backend")
	if err != nil {
		t.Fatal(err)
	}
	if h != nil {
...
func TestStart_EnabledDoesNotRequireStepBinary(t *testing.T) {
	...
	_, err := Start(context.Background(), slog.Default(), "alt-backend")
	if err == nil {
...
func TestStartWith_EnabledEnrollsAndStops(t *testing.T) {
	...
	cfg := &Config{
		Mode:            ModeEnabled,
		Subject:         "alt-data-hub",
		SANs:            []string{"alt-data-hub"},
		CertPath:        filepath.Join(dir, "svc-cert.pem"),
		KeyPath:         filepath.Join(dir, "svc-key.pem"),
		Provisioner:     "pki-agent-alt-data-hub",
		PasswordFile:    "/run/secrets/pki-agent-alt-data-hub-jwk",
...
func TestStart_EnabledSharedSecretRejected(t *testing.T) {
	...
	if _, err := Start(context.Background(), slog.Default(), "alt-backend"); err == nil {
