// Package pki is the in-process certificate lifecycle for the alt-backend
// module (cmd/backend, cmd/harvester, cmd/datahub).
//
// It is intentionally independent of the pki-agent module: no replace
// directive, no shared Go import. The state machine, atomic writes and
// subject-scoped config live here and are tested with a fake issuer, fake
// clock and a temp filesystem. Minting against step-ca uses NativeStepCAIssuer
// (official jose + provision token APIs, no step CLI) when PKI_ENROLLMENT=enabled.
package pki
