...
const (
	...
	liveCAContainer = "alt-pki-native-itest-preproc"
	liveCANetwork   = "alt-pki-native-itest-preproc-net"
	liveCAHostPort  = "19011"
	liveCAPassword  = "itest-only"
	liveJWKPassword = "itest-jwk-pre-processor"
	liveProvisioner = "pki-agent-pre-processor"
	liveSubject     = "pre-processor"
)
...
//
//	docker network create alt-pki-native-itest-preproc-net
//	docker run --rm -d --name alt-pki-native-itest-preproc \
//	  --network alt-pki-native-itest-preproc-net \
//	  -p 127.0.0.1:19011:9000 \
...
//	  -e DOCKER_STEPCA_INIT_WITH_CA_URL=https://127.0.0.1:19011 \
...
func TestNativeStepCAIssuer_DisposableLiveCA(t *testing.T) {
	...
	passwordFile := writePasswordFile(t, tmp, "pki-agent-pre-processor-jwk", liveJWKPassword, 0o400)
...
printf '%%s\n' %q > /tmp/pki-agent-pre-processor-jwk
chmod 400 /tmp/pki-agent-pre-processor-jwk
step ca provisioner add %q --type JWK --create \
  --password-file /tmp/pki-agent-pre-processor-jwk \
