...
func TestStart_DisabledLogs(t *testing.T) {
	...
	h, err := Start(context.Background(), log, "pre-processor")
	if err != nil {
		t.Fatal(err)
	}
	if h != nil {
...
func TestStart_EnabledDoesNotRequireStepBinary(t *testing.T) {
	...
	_, err := Start(context.Background(), slog.Default(), "pre-processor")
	if err == nil {
...
func TestStartWith_EnabledEnrollsAndStops(t *testing.T) {
	...
	cfg := &Config{
		Mode:            ModeEnabled,
		Subject:         "pre-processor",
		SANs:            []string{"pre-processor"},
		CertPath:        filepath.Join(dir, "svc-cert.pem"),
		KeyPath:         filepath.Join(dir, "svc-key.pem"),
		Provisioner:     "pki-agent-pre-processor",
		PasswordFile:    "/run/secrets/pki-agent-pre-processor-jwk",
...
func TestStart_EnabledSharedSecretRejected(t *testing.T) {
	...
	if _, err := Start(context.Background(), slog.Default(), "pre-processor"); err == nil {
