...
func (s *healthCheckerService) CheckNewsCreatorHealth(ctx context.Context) error {
	...
	if resp.StatusCode != http.StatusOK {
		s.logger.ErrorContext(ctx, "news creator not healthy", "status", resp.StatusCode)
	...
	if err := json.NewDecoder(resp.Body).Decode(&response); err != nil {
		s.logger.ErrorContext(ctx, "failed to decode health response", "error", err)
...
func (s *healthCheckerService) WaitForHealthy(ctx context.Context) error {
	s.logger.DebugContext(ctx, "waiting for news creator to become healthy")
	...
	if err := s.CheckNewsCreatorHealth(ctx); err == nil {
		s.logger.DebugContext(ctx, "news creator is now healthy")
		return nil
	}
	...
	for {
		select {
		case <-ctx.Done():
			s.logger.ErrorContext(ctx, "context canceled while waiting for health")
			return ctx.Err()
		case <-ticker.C:
			if err := s.CheckNewsCreatorHealth(ctx); err == nil {
				s.logger.DebugContext(ctx, "news creator is now healthy")
