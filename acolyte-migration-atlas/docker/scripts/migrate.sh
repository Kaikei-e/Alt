#!/bin/sh
# Atlas Migration Script for Acolyte DB
...
check_requirements() {
    ...
    if [ -z "$DATABASE_URL" ] && [ -n "${ACOLYTE_DB_HOST:-}" ]; then
        log_info "Constructing DATABASE_URL from environment variables..."
        local user="${ACOLYTE_DB_USER:-postgres}"
        local name="${ACOLYTE_DB_NAME:-postgres}"
        local port="${ACOLYTE_DB_PORT:-5432}"
        local password=""
        if [ -n "${ACOLYTE_DB_PASSWORD_FILE:-}" ] && [ -f "$ACOLYTE_DB_PASSWORD_FILE" ]; then
            password=$(cat "$ACOLYTE_DB_PASSWORD_FILE")
        else
            password="${ACOLYTE_DB_PASSWORD:-}"
        fi
        DATABASE_URL="postgres://$user:$password@$ACOLYTE_DB_HOST:$port/$name?sslmode=disable&search_path=public"
        export DATABASE_URL
    fi
...
main() {
    ...
    log_info "Atlas Migration Manager for Acolyte DB"
