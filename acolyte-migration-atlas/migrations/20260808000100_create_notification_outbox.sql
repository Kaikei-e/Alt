...
--
-- This table lives in acolyte-db, not in alt-db, and that placement is
...
-- business state change share one local commit. The report_jobs queue lives here,
...
--
-- A relay in acolyte-orchestrator forwards rows to alt-data-hub over mTLS and marks
-- them forwarded. Delivery to the device is tracked separately, in alt-db's
-- push_deliveries.
CREATE TABLE notification_outbox (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    -- Derived from the business fact (e.g. 'report:<job_id>'), never generated
...
COMMENT ON TABLE notification_outbox IS 'Transactional outbox for push notifications (forwarded to alt-data-hub by the acolyte-orchestrator relay)';
